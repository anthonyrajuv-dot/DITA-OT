...
<!--        <fo:static-content flow-name="odd-frontmatter-header">
    ...
    <xsl:template name="insertAnswerKeyStaticContents">
        <xsl:call-template name="insertAnswerKeyOddFooter"/>
        <xsl:call-template name="insertAnswerKeyOddHeader"/>
    </xsl:template>
    
    <xsl:template name="insertAnswerKeyOddHeader">
        <fo:static-content flow-name="odd-answer-key-header">
            <fo:block text-align="center" margin-top="12pt">
                <xsl:apply-templates 
                    select="$map/descendant::*[contains(@class,' bookmap/mainbooktitle ')]/*"/>
            </fo:block>
        </fo:static-content>
    </xsl:template>
    
    <xsl:template name="insertAnswerKeyOddFooter">
        <fo:static-content flow-name="odd-answer-key-footer">
            <fo:block text-align="center" margin-top="12pt">
                <fo:page-number/>
            </fo:block>
        </fo:static-content>
