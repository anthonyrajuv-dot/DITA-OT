...
-->
    ...
    <!-- Also used by new_assessment_from_q. (Updated by AnthonyR for multichoice)-->
	 <xsl:template match="lcSingleSelect2 | lcTrueFalse2 | lcMultipleSelect2" mode="new_learning_question">
        <xsl:param name="unit"/>
        <xsl:param name="q_number"/>
        <xsl:param name="task_number"/>
        ...
        <itemBody xmlns="http://www.imsglobal.org/xsd/imsqti_v2p1">
			...
			<xsl:for-each select="lcOpenAnswer2/data">				
				...
				<data name="sectionDefination">				
					...
					<xsl:attribute name="datatype">
						<xsl:value-of select="concat('S',$filename_ID)"/>
					</xsl:attribute>
				</data>
				</sectionItem>
			</xsl:for-each>
