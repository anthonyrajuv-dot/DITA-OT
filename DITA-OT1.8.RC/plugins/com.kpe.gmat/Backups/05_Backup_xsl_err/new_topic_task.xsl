...
-->
        ...
        <data name="questionMetadata">
            ...
            <xsl:attribute name="value">
                <xsl:text>media/download?mediaId=174935&amp;filename=Sample_PDF_A</xsl:text>
            </xsl:attribute>
            <xsl:attribute name="datatype">
                <xsl:choose>
                    <xsl:when test="@test_id and @test_id != ''">
        ...
        <data name="questionMetadata">
            <xsl:attribute name="key">
                <xsl:text>sampleQuestionUrl</xsl:text>
            </xsl:attribute>
            <xsl:attribute name="value">
                <xsl:text>media/download?mediaId=174935&amp;filename=Sample_PDF_B</xsl:text>
            </xsl:attribute>
            <xsl:attribute name="datatype">
                <xsl:choose>
                    <xsl:when test="@test_id and @test_id != ''">
    ...
    <xsl:template match="difficulty">
        ...
        <xsl:choose>
            ...
            <xsl:when test="@id = 'questionNumber'">
                <data name="questionMetadata">
                    <xsl:attribute name="key">
                        <xsl:value-of select="@id"/>
                    </xsl:attribute>
	...
	<xsl:template match="essayData">
		<xsl:param name="q_number"/>
		<xsl:choose>
			<xsl:when test="@state and @month and @year">
				<data name="questionAttribute">
					<xsl:attribute name="key">
				...
				<data name="questionMetadata">
					...
					<xsl:attribute name="value">
						<xsl:value-of select="functx:questionNumber(@state,@year,@month,$q_number)"/>
					</xsl:attribute>
					<xsl:attribute name="datatype">
						<xsl:value-of select="concat('Q',$q_number)"/>
					</xsl:attribute>
	...
	<xsl:template match="sampleQuestion | sampleAnswer">
		<xsl:param name="q_number"/>
		<xsl:choose>
			<xsl:when test="@url">
				<data name="questionMetadata">
					<xsl:attribute name="key">
						<xsl:if test="self::sampleQuestion">
							<xsl:text>questionUrl</xsl:text>
						</xsl:if>
						<xsl:if test="self::sampleAnswer">
    ...
    <xsl:template match="*[contains(@class,' topic/topic ')]" mode="new_learning_question">
		...
		<xsl:choose>
			<xsl:when test="/kpe-concept/kpe-conceptBody[1]/sectioncontains(@outputclass,'barsection')">
		        
        <assessmentItem xmlns="http://www.imsglobal.org/xsd/imsqti_v2p1"
            xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
            xsi:schemaLocation="http://www.imsglobal.org/xsd/imsqti_v2p1 http://www.imsglobal.org/xsd/imsqti_v2p1.xsd"
            identifier="{$identifier}" title="section" timeDependent="false" adaptive="false">
            <xsl:value-of select="$newline"/>
        ...
        <xsl:message>file_id_base is: <xsl:value-of select="$file_id_base"/>.</xsl:message>
        	...
        	<xsl:variable name="bar_section" select="//*/kpe-concept/kpe-conceptBody[1]/section/@outputclass"/>
        	<xsl:variable name="mappedValue" select="//*/kpe-concept/kpe-conceptBody[1]/section/@points"/>
            ...
            <mapping defaultValue="0">
                <sectionEntry xmlns="http://www.imsglobal.org/xsd/imsqti_v2p1/" 
                					mapKey="PERCENTAGE" 
                					mappedValue="{$mappedValue}"></sectionEntry>
            </mapping>
        </responseDeclaration>
		<itemBody>
			<!--Bar Section-->
    ...
    <xsl:template match="lcOpenQuestion2" mode="new_learning_question">
        ...
        <itemBody xmlns="http://www.imsglobal.org/xsd/imsqti_v2p1">
			...
			<!-- BAR Essay	 -->
			<sectionGroup>
			<xsl:for-each select="lcOpenAnswer2/data">				
				...
				<sectionItem href="{$filename_ext}">
				<data name="sectionDefination">				
					<!--<xsl:attribute name="value"><xsl:value-of select="concat('../los/',$los_filename,'.dita')"/></xsl:attribute>-->
					<xsl:attribute name="datatype">
						<xsl:value-of select="concat('S',count(preceding-sibling::data) + 1)"/>
					</xsl:attribute>
				</data>
				</sectionItem>
			</xsl:for-each>
