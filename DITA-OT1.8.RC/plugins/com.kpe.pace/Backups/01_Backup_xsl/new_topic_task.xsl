...
-->
        ...
        <xsl:variable name="out_file" select="concat('questions/',$file_name)"/>
		
		
		
        <xsl:message>(new_learning_question) out_file is "<xsl:value-of select="$out_file"
            />".</xsl:message>
        ...
        <questionItem>
            ...
            <!-- [SP] 2022-08-24 sfb: Changes to handle multiple topicsubject elements
            ...
            <xsl:apply-templates select="topicsubject" mode="find_los">
                ...
                <xsl:with-param name="q_id_base" select="$file_id_base"/>
...
-->
        ...
        <data name="learningObjective">
            <xsl:attribute name="value" select="$file_name"/>
...
-->
                ...
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        ...
                        <prompt>
							...
							<sectionGroup>
						<sectionItem href="{$question_item}">
						<xsl:apply-templates select="$topicref/topicsubject" mode="new_question"/>
						...
						<xsl:attribute name="datatype">
							<xsl:value-of select="concat('S',$q_number)"/>
						</xsl:attribute>
					</data> -->
					</sectionItem>
					</sectionGroup>						
	...
	<!-- BAR Section 24-06-2024 -->
	<xsl:template match="lcNextSteps" mode="new_learning_question">
        <xsl:param name="unit"/>
        <xsl:param name="q_number"/>
        <xsl:param name="task_number"/>
        ...
        <responseDeclaration xmlns="http://www.imsglobal.org/xsd/imsqti_v2p1" cardinality="single">
            ...
            <mapping defaultValue="0">
                <xsl:apply-templates select="lcNextSteps" mode="get_mapEntry_open"/>
            </mapping>
        </responseDeclaration>
        <itemBody xmlns="http://www.imsglobal.org/xsd/imsqti_v2p1">            
                <xsl:choose>
                    <xsl:when test="lcNextSteps/node()[self::text()]">
                                <xsl:apply-templates select="lcNextSteps"/>                        
                    </xsl:when>
                    <xsl:otherwise>
                       <xsl:apply-templates select="lcNextSteps"/>                        
                    </xsl:otherwise>
    ...
    <xsl:template match="lcSingleSelect2 | lcTrueFalse2" mode="new_multipart_learning_question">
        ...
        <choiceInteraction maxChoices="1" shuffle="true">
...
-->
    <xsl:template match="lcOpenQuestion2" mode="new_multipart_learning_question">
        ...
        <extendedTextInteraction minStrings="1" textFormat="plain">
        	...
        	<xsl:choose>
        		<xsl:when test="lcOpenAnswer2/node()[self::text()]">
        			<prompt>
        				<p>
        					<xsl:apply-templates select="lcOpenAnswer2"/>
        				</p>
        			</prompt>
        		</xsl:when>
        		<xsl:otherwise>
        			<SectionGroup>
        				<xsl:apply-templates select="lcOpenAnswer2/*" mode="identity">
        					<xsl:with-param name="type" select="'question'"/>
        				</xsl:apply-templates>
        			</SectionGroup>
        		</xsl:otherwise>
...
-->
    ...
    <!--Securities additions for color-->
    <xsl:template match="sample [@othertype = 'example']" mode="#all" priority="100">
        <xsl:param name="type"/>
    ...
    <xsl:template match="sample [@othertype = 'take_note']" mode="#all" priority="100">
        <xsl:param name="type"/>
    ...
    <xsl:template match="sample [@othertype = 'test_topic_alert']" mode="#all" priority="100">
        <xsl:param name="type"/>
    ...
    <xsl:template match="p[@outputclass = 'center']" mode="#all">
	 ...
	 <!--BAR Section  24-06-2024 -->
	<xsl:template match="p[@outputclass = 'issue']" mode="#all">
        <issue>
		<p>
            <xsl:apply-templates/>
		</p>
	...
	<xsl:template match="p[@outputclass = 'rules']" mode="#all">
        <rules>
		<p>
            <xsl:apply-templates/>
		</p>
	...
	<xsl:template match="ol[@outputclass = 'analysis']" mode="#all">
        <analysis>
		<ol>
	...
	<xsl:template match="p[@outputclass = 'conclusion']" mode="#all">
        <conclusion>
		<p>
            <xsl:apply-templates/>
		</p>
