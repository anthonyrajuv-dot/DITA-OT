...
<!--                    <xsl:for-each select="following-sibling::examMap[@assessment_type!='test-exam-primary']">-->
    ...
    <xsl:template name="get_file_id_base">
        ...
        <!-- [SP] 2018-05-08 sfb: Merging results in an unwanted "_merge" in the filename. Strip it first. -->
        <xsl:variable name="no_merge" select="replace(@orig_href,'_merge','')"/>
        
        <xsl:variable name="file_id_base" select="replace($no_merge,'.*/([^/]*)\.dita','$1')"/>
...
<!--        <xsl:variable name="file_name" select="concat('overview/',$name_base,'_TOPIC',$topic_number,'.dita')"/>-->
    ...
    <xsl:template match="topichead" mode="reltable">
        <xsl:apply-templates select="topichead | topicref | examMap" mode="reltable"/>
    </xsl:template>
    
    <xsl:template match="examMap" mode="reltable">
        <!--        orig_href="schwesernotes/book_2/questions/cfaL1_question_00001_133.dita""-->
        ...
        <xsl:for-each select="topicref/topicsubject">
            <relrow>
                <relcell>
                    <topicref>
                        <xsl:attribute name="href" select="concat('questions/',$file_id_base,'.dita')"/>
                    </topicref>
                </relcell>
                <relcell/>
                <relcell>
                    <topicref>
                        <xsl:attribute name="href" select="replace(@orig_href,'^.*/los/','los/')"/>
                    </topicref>
                </relcell>
            </relrow>
        </xsl:for-each>
    ...
    <xsl:template match="topicref" mode="reltable">
<!--        <xsl:comment>@orig_href is <xsl:value-of select="@orig_href"/></xsl:comment>-->
        <xsl:variable name="href">
            <xsl:choose>
                <xsl:when test="contains(@orig_href,'_merge.dita')">
                    <xsl:value-of select="replace(@orig_href,'^.*/?([^/]*)/([^/]*)_merge.dita','$1/$2.dita')"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of select="replace(@orig_href,'^.*/?([^/]*)/([^.]*).dita','$1/$2.dita')"/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
        ...
        <xsl:for-each select="topicsubject">
            <relrow>
                <relcell>
                    <topicref>
                        <!-- [SP] 2018-08-06 sfb: Replace all folders with topics. -->
                        <xsl:attribute name="href" select="replace($href,'^[^/]*/([^.]*)\.dita$','topics/$1.dita')"/>
                    </topicref>
                </relcell>
                <relcell/>
                <relcell>
                    <topicref>
                        <xsl:attribute name="href" select="replace(@href,'^.*/los/','los/')"/>
                    </topicref>
                </relcell>
            </relrow>
        </xsl:for-each>
...
<!--
    NOTE: Much of this was built out for the first or second pass through PACE, but abandoned.
          Now (in PACE ref IV) we're building the reltable a bit more simply. 
