...
-->
        ...
        <xsl:choose>
            <xsl:when
                test="contains(@class, ' kpe-assessmentOverview/kpe-assessmentOverview ') and $OUTPUT_TYPE = 'testbank'">
    ...
    <xsl:template name="getTopicrefShortdesc">
        <xsl:variable name="id"
            select="ancestor-or-self::*[contains(@class, ' topic/topic ') or contains(@class, ' topic/section ')][1]/@id"/>
    ...
    <xsl:template name="processTopicChapter">
        ...
        <xsl:variable name="title"
            select="$mapTopicref/*[contains(@class, ' map/topicmeta ')][1]/*[contains(@class, ' topic/navtitle ')][1]/text()"/>
        <fo:page-sequence master-reference="body-sequence"
            ...
            <fo:flow flow-name="xsl-region-body">
                <fo:block xsl:use-attribute-sets="topic">
                    ...
                    <xsl:apply-templates select="*[contains(@class, ' topic/prolog ')]"/>
                    <!-- [SP] No need for chapter static content in generic spdf4 format. -->
...
-->
                    <fo:block xsl:use-attribute-sets="topic.title">
                        ...
                        <fo:block>
                            ...
                            <xsl:choose>
                                <xsl:when
                                    test="contains($title, 'Exam') or contains($title, 'Quiz')">
                                    <!--do nothing-->
                                </xsl:when>
                                <xsl:otherwise>
        ...
        <fo:page-sequence master-reference="body-sequence"
            ...
            <fo:flow flow-name="xsl-region-body">
                <fo:block xsl:use-attribute-sets="topic">
                    ...
                    <xsl:variable name="part_image"
                        select="$topicParts/*[@id = $id]/topicmeta/data/image"/>
                    <xsl:variable name="part_image_href" select="$part_image/@href"/>
                    <xsl:if test="$part_image_href != ''">
                        <xsl:variable name="part_image_path"
                            select="concat($input.dir.url, $part_image_href)"/>
                        ...
                        <fo:block text-align="center">
                            <fo:external-graphic src="url({$part_image_path})"
                                xsl:use-attribute-sets="image" content-width="{$image_width}"/>
                        </fo:block>
                    </xsl:if>
                    ...
                    <xsl:apply-templates select="*[contains(@class, ' topic/prolog ')]"/>
                    <!-- [SP] No need for part static content in generic spdf4 format. -->
                    <!--				    <xsl:call-template name="insertChapterFirstpageStaticContent">
...
-->
    ...
    <xsl:template name="processTopicFinalExam">
        <fo:page-sequence master-reference="final-exam-sequence" format="1"
            xsl:use-attribute-sets="__force__page__count">
            <!--            <xsl:call-template name="insertColophonStaticContents"/>-->
            <xsl:call-template name="insertBodyStaticContents"/>
            <fo:flow flow-name="xsl-region-body">
...
-->
                    ...
                    <fo:block xsl:use-attribute-sets="final.exam.partno">
                        <xsl:value-of
                            select="
                                $map/*[contains(@class, ' bookmap/bookmeta ')]
                    ...
                    <fo:block xsl:use-attribute-sets="final.exam.title" text-align="left">
                        ...
                        <xsl:choose>
                            <xsl:when test="contains($OUTPUT_TYPE, 'final_alt')">
                                <xsl:value-of
                                    select="$map//*[contains(@class, ' bookmap/booktitlealt ')][1]"
                                />
                            </xsl:when>
                            <xsl:otherwise>
                                <xsl:for-each select="child::*[contains(@class, ' topic/title ')]">
            ...
            <fo:flow flow-name="xsl-region-body">
                ...
                <fo:block xsl:use-attribute-sets="topic">
                    ...
                    <fo:block xsl:use-attribute-sets="final.exam.title" text-align="left">
                        ...
                        <xsl:value-of
                            select="
                                $map/*[contains(@class, ' bookmap/booktitle ')]
...
-->
        ...
        <xsl:variable name="attrSet1A"
            select="concat(substring-before($attrSet1, '.topic.title'), '.section.title')"/>
        ...
        <fo:block>
            ...
            <xsl:call-template name="processAttrSetReflection">
                ...
                <xsl:with-param name="path" select="'../../cfg/fo/attrs/spdf4_commons-attr.xsl'"/>
            </xsl:call-template>
...
-->
        ...
        <xsl:choose>
            <xsl:when
                test="($initial_count + $part_correction + $bookmap_correction + $overview_correction) &gt; 4">
                <xsl:value-of
                    select="$initial_count + $part_correction + $bookmap_correction + $overview_correction - 1"
                />
            </xsl:when>
            ...
            <xsl:otherwise>
                <xsl:value-of
                    select="$initial_count + $part_correction + $bookmap_correction + $overview_correction"
                />
            </xsl:otherwise>
        </xsl:choose>
    ...
    <xsl:template name="insertChapterLabel">
        ...
        <xsl:choose>
            ...
            <xsl:when test="$type = 'part'">
                <fo:block text-align="center"
                    xsl:use-attribute-sets="__part__frontmatter__name__inline">
                    <!--<xsl:call-template name="insertVariable">
    ...
    <xsl:template match="*[contains(@class, ' topic/topic ')]">
        ...
        <xsl:variable name="exam_type">
            <xsl:choose>
                <xsl:when
                    test="descendant-or-self::*[contains(@class, ' kpe-commonMeta-d/lmsCategory ') and @value = 'test_exam_primary']">
                    <xsl:text>test_exam_primary</xsl:text>
                </xsl:when>
                <xsl:when
                    test="descendant-or-self::*[contains(@class, ' kpe-commonMeta-d/lmsCategory ') and @value != 'test_exam_primary']">
                    <xsl:value-of
                        select="descendant-or-self::*[contains(@class, ' kpe-commonMeta-d/lmsCategory ')]/@value"
                    />
                </xsl:when>
...
-->
                ...
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        <xsl:choose>
                            <!-- [SP] omit questions -->
                            <xsl:when
                                test="contains(@class, ' kpe-assessmentOverview/kpe-assessmentOverview ') and $OUTPUT_TYPE = 'course_no_questions'">
                                <!-- do nothing -->
                            </xsl:when>
    ...
    <xsl:template match="*[contains(@class, ' topic/p ')]">
        ...
        <xsl:choose>
            ...
            <xsl:when test="parent::*[contains(@class, ' topic/lq ')]">
                <fo:block xsl:use-attribute-sets="lq" id="{@id}">
                    ...
                    <xsl:choose>
                        <xsl:when
                            test="
                                following-sibling::*[contains(@class, ' topic/lq ')]
                                or preceding-sibling::*[contains(@class, ' topic/lq ')]">
                            <!-- Don't add anything. -->
                        </xsl:when>
                        <xsl:when
                            test="count(preceding-sibling::*[contains(@class, ' topic/p ')]) = 0">
            ...
            <xsl:when
                test="parent::*[contains(@class, ' topic/note ')] and count(preceding-sibling::*[contains(@class, ' topic/p ')]) = 0">
    ...
    <xsl:template match="*" mode="placeImage">
        ...
        <xsl:variable name="imageAlignTested">
            <xsl:choose>
                ...
                <!-- If not specified, use the $image-align value from spdf4_BasicSettings.xsl. -->
    ...
    <xsl:template match="*[contains(@class, ' learning2-d/lcQuestion2 ')]">
        ...
        <fo:block xsl:use-attribute-sets="lcQuestion2" id="{@id}">
            ...
            <xsl:variable name="item_number"
                select="count(ancestor::*[contains(@class, ' kpe-question/kpe-question ')][1]/preceding-sibling::*[contains(@class, ' kpe-question/kpe-question ')]) + 1"/>
...
-->
                        ...
                        <xsl:variable name="legacy_id"
                            select="ancestor-or-self::*[contains(@class, ' kpe-question/kpe-question ')]/descendant::*[contains(@class, ' kpe-commonMeta-d/legacyID ')]/@value"/>
            ...
            <xsl:choose>
                ...
                <xsl:otherwise>
                    ...
                    <xsl:variable name="unit_id"
                        select="ancestor::*[contains(@class, ' topic/topic ')][position() = last()]/@id"/>
                    <xsl:value-of
                        select="count(preceding::*[contains(@class, ' topic/fn ') and not(@callout) and ancestor::*[@id = $unit_id]]) + 1"
                    />
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
