...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fo="http://www.w3.org/1999/XSL/Format"
    xmlns:dita2xslfo="http://dita-ot.sourceforge.net/ns/200910/dita2xslfo" xmlns:opentopic="http://www.idiominc.com/opentopic" version="2.0">
    ...
    <xsl:template match="*" mode="commonTopicProcessing">
        ...
        <xsl:choose>
            <xsl:when test="contains(@class, ' kpe-assessmentOverview/kpe-assessmentOverview ') and $OUTPUT_TYPE = 'testbank'">
                ...
                <xsl:apply-templates select="$mapTopicref/@navtitle" mode="processTopicTitle"/>
        ...
        <xsl:if test="$KPE_FILENAMES and contains(@class,'kpe-')">
            <fo:list-block keep-with-next.within-page="always">
                <fo:list-item>
                    <fo:list-item-label end-indent="150pt">
                    ...
                    <fo:list-item-body start-indent="body-start()">
                        <xsl:if test="contains(@class,' kpe-question/kpe-question ')">
                            <xsl:if test="$mapTopicref/*[contains(@class,' topic/data ')]">
                                <xsl:variable name="data_value">
                                    <xsl:apply-templates select="$mapTopicref/*[contains(@class,' topic/data ')]/text()"/>                    
                                </xsl:variable>
                                <fo:block xsl:use-attribute-sets="p.object" text-align="right">
                                    <xsl:value-of select="concat($data_value, ' question')"/>
                                </fo:block>
                            </xsl:if>
                        </xsl:if>
                    </fo:list-item-body>
...
-->
                    <fo:block xsl:use-attribute-sets="topic.title">
                        ...
                        <xsl:if test="$KPE_FILENAMES">
                            <fo:block font-size="12pt">
                                <xsl:call-template name="insertVariable">
                                    <xsl:with-param name="theVariableID" select="'Word count'"/>
                                    <xsl:with-param name="theParameters">
                                        <number>
                                            <xsl:value-of select="count(*//*[contains(@class,' topic/body ')]//text()/tokenize(., '\W+')[.!=''])"/>
                                        </number>
                                    </xsl:with-param>
                                </xsl:call-template>
                            </fo:block>
...
-->
    ...
    <!-- These templates are here to reduce the noise in the log. -->
    <xsl:template match="*[contains(@class,' map/topicref ')]" mode="topicTitleNumber">
        <!-- Do nothing. -->
    </xsl:template>
    <xsl:template match="*[contains(@class,' map/topichead ')]" mode="topicTitleNumber">
        <!-- Do nothing. -->
    </xsl:template>
