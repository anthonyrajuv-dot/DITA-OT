...
-->
        ...
        <xsl:choose>
        	...
        	<xsl:when test="(contains(@class,' kpe-concept/kpe-concept ')) and not(contains(@class, ' kpe-assessmentOverview '))">
        		<!-- ARV added on 05-09-2025 -->
        		<xsl:apply-templates select="*[contains(@class, ' topic/title ')]"/>
        	</xsl:when>
            <xsl:otherwise>
            	<!-- ARV added on 05-09-2025 -->
            	<xsl:if test="(contains(@class, ' kpe-assessmentOverview/kpe-assessmentOverview ')) or (contains(@class, ' map/map '))">
            		<xsl:apply-templates select="*[contains(@class, ' topic/title ')]"/>
            	</xsl:if>
            </xsl:otherwise>
        </xsl:choose>
...
-->
            ...
            <!--ARV:14/08/2025 <xsl:when test="$topicType = 'topicChapter' and $OUTPUT_TYPE = 'testbank'">
            ...
            <!--ARV:14/08/2025 <xsl:when test="$topicType = 'topicChapter'">
                <xsl:call-template name="processTopicChapter"/>
            </xsl:when>
        	...
        	<!--ARV:14/08/2025 <xsl:when test="$topicType = 'topicNotices'">
                ...
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        <xsl:choose>
                            ...
                            <xsl:otherwise>
                                <xsl:message>ARV Test: <xsl:value-of select="@class"/></xsl:message>
                                <xsl:apply-templates select="." mode="processTopic"/>
                            </xsl:otherwise>
                        </xsl:choose>
                    </xsl:otherwise>
