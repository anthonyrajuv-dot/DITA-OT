...
<!--          <xsl:message> + [DEBUG ]  currentContainer != @containerType, currentPara=<xsl:sequence select="local:reportPara(.)"/></xsl:message>-->
          <xsl:element name="{@containerType}">
            ...
            <xsl:if test="@containerOthertype">
              <xsl:attribute name="othertype" select="string(@containerOthertype)"/>
            </xsl:if>
...
<!--  CHANGE TOPIC NAME HERE  -->
    <!--PSB edited  <xsl:variable name="result" select="concat($fileNamePrefix, 'topic', string-join($treePosString, ''))"/> -->
    <xsl:variable name="result" select="concat($fileNamePrefix,'caia_key_concept_00001_', string-join($treePosString, ''))"/>
    <xsl:sequence select="$result"/>
    ...
    <xsl:if test="$para">
      <xsl:sequence 
        select="concat('[', 
                       ...
                       if ($para/@containerOthertype)
                          then concat(' containerOthertype=', $para/@containerOthertype)
                          else '',
                          ']',
