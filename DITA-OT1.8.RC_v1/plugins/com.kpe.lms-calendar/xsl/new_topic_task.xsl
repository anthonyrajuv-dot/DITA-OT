...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="*[contains(@class,' topic/topic ')]" mode="new_topic">
        ...
        <topic>
            ...
            <title>
                <xsl:choose>
                    <!-- [SP] 20150-06-18 sfb: For calendar transform, we need to use the title in the topic, not navtitles. -->
...
-->                    <xsl:when test="*[contains(@class,' topic/titlealts ')]/*[contains(@class,' topic/navtitle ')]">
            ...
            <prolog>
                <metadata>
                    ...
                    <!-- [SP] 2015-04-08: It's back...under conditions. -->
                    
                   <othermeta name="duration">
                       <xsl:attribute name="content">
                           <xsl:choose>
                               <xsl:when test="$show_duration = true() and descendant::duration[1]/@value">
                    ...
                    <!-- Handle other metadata (other than duration).-->
                    <xsl:apply-templates select="descendant::metadata[not(descendant-or-self::duration)]" mode="get_topic_metadata"/>
                </metadata>
            </prolog>
            <!-- Karissa says no! -->
            <!--            <shortdesc>
...
<!--        <xsl:param name="lesson_number"/>-->
        ...
        <xsl:variable name="label">
            <xsl:call-template name="get_label">
                <xsl:with-param name="task_category" select="descendant-or-self::lmsCategory[1]/@value"/>
            </xsl:call-template>
        </xsl:variable>
        ...
        <kaptask>
            ...
            <prolog>
                <metadata>
                    <xsl:choose>
                        <xsl:when test="$task_category = 'reading'">
    ...
    <!-- #### NEW QUESTION ###################################################################### -->
    <!-- I suspect that this is no longer used. -->
...
<!--            <xsl:apply-templates select="lcFeedbackCorrect2"/>-->
    ...
    <xsl:template match="@*" mode="vignette">
        ...
        <xsl:if test="name() != 'xtrf' and name() != 'xtrc' and name() != 'class'">
...
<!--            <xsl:call-template name="get_correct_single"/>-->
    ...
    <xsl:template match="lcOpenQuestion2" mode="new_learning_question">
        ...
        <responseDeclaration xmlns="http://www.imsglobal.org/xsd/imsqti_v2p1"
            identifier="{concat('Q',$q_number)}" cardinality="single">
...
-->
        ...
        <xsl:message>content of nodes is <xsl:copy-of select="$nodes"/></xsl:message>-->
            ...
            <xsl:otherwise>
                <xsl:comment>lcFeedbackCorrect2 containing elements.</xsl:comment>
...
<!--                <xsl:value-of select="$topic_id"/>-->
            ...
            <xsl:choose>
                ...
                <xsl:otherwise>
<!-- [SP] 2-15-06-18 sfb: I think this change is right, but I'm not 100% sure. -->
<!--                    <title>
...
-->        
        <p><sup><xsl:value-of select="$topic_pos + $fn_offset"/></sup><xsl:apply-templates mode="identity"/></p>
    </xsl:template>
    
    <xsl:template name="get_fn_offset">
        <xsl:variable name="topic_id" select="ancestor::*[contains(@class,' topic/topic ')]/@id"/>
        <xsl:variable name="topicref" select="$supermap/descendant::topicref[contains(@orig_href,concat($topic_id,'.dita'))]"/>
...
<!--        <xsl:message>prev_fn_counts for <xsl:value-of select="$topic_id"/> is "<xsl:value-of select="$prev_fn_counts"/>".</xsl:message>
    ...
    <xsl:template match="label" mode="identity" priority="100">
        <p><strong><xsl:apply-templates mode="identity"/></strong></p>
    </xsl:template>
...
<!--        <data name="ucmsId">
    ...
    <xsl:template match="title" mode="ital_title" priority="100">
        <p><em><xsl:apply-templates mode="identity"/></em></p>
    </xsl:template>
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:copy>
                    ...
                    <xsl:apply-templates select="@*[not(name() = 'href') and not(name()='outputclass') and not(name()='placement')]|node()" mode="identity">
                        <xsl:with-param name="type" select="$type"/>
                    </xsl:apply-templates>
                </xsl:copy>
            </xsl:otherwise>
...
-->        
    ...
    <xsl:template match="equation-block" mode="#all" priority="100">
        <xsl:param name="type"/>
    ...
    <xsl:template match="ol" mode="identity" priority="100">
        <xsl:copy>
            <xsl:choose>
    ...
    <!-- [KPE] 2015-04-14 sp: Added processing for <desc> to <p> -->
    ...
    <xsl:template match="ph" mode="identity" priority="100">
        <xsl:choose>
...
<!--            <xsl:choose>
        ...
        <sup><xsl:value-of select="$topic_pos + $fn_offset"/></sup>
...
-->
    ...
    <xsl:template match="tgroup" mode="PACE_table">
        <table>
    ...
    <xsl:template match="thead" mode="PACE_table">
        <xsl:apply-templates select="row" mode="PACE_table_head"/>
    </xsl:template>
    <xsl:template match="row" mode="PACE_table_head">
        <tr>
            <xsl:apply-templates select="entry" mode="PACE_table_head"/>
        </tr>
    </xsl:template>
    <xsl:template match="entry" mode="PACE_table_head">
        <th bgcolor="#cccccc">
            <xsl:choose>
                <xsl:when test="@align">
    ...
    <xsl:template match="tbody" mode="PACE_table">
        <xsl:apply-templates select="row" mode="PACE_table_body"/>
    </xsl:template>
    <xsl:template match="row" mode="PACE_table_body">
        <tr>
    ...
    <xsl:template match="entry" mode="PACE_table_body">
...
<!-- ===================================================================== -->
    ...
    <xsl:template match="*[contains(@class,' topic/dt ')]" mode="identity" priority="100" xml:space="default">
        <p><strong>
        <xsl:choose>
            <xsl:when test="count(ancestor::*[contains(@class,' topic/dl ')]) = 2" xml:space="default">
                <em><xsl:apply-templates mode="identity"/></em>
            </xsl:when>
            <xsl:otherwise>
                <xsl:apply-templates mode="identity"/>
            </xsl:otherwise>
...
<!--            <xsl:message>determining target for <xsl:value-of select="@href"/>.</xsl:message>-->
    ...
    <xsl:template match="term" mode="identity" priority="100">
        <xsl:param name="type"/>
        <strong><xsl:apply-templates mode="identity"/></strong>
    </xsl:template>
    ...
    <!--sfb: need to check with Peyton about this, it outputs data, then an empty CDATA object. -->
    ...
    <xsl:template match="*[contains(@class,' kpe-common-d/emphasisBold ')]|emphasisBold" mode="#all" priority="100">
        <strong><xsl:apply-templates/></strong>
    </xsl:template>

    <xsl:template match="*[contains(@class,' kpe-common-d/emphasisItalics ')]|emphasisItalics" mode="#all" priority="100">
...
<!--1/14/2016 PSB: Added commandWord processing-->
