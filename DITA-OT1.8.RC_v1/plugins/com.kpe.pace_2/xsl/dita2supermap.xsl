...
<!--        <xsl:variable name="chunk_count" select="count($topic//section[title]) + count($topic//data[@name='pace_break' and upper-case(@value)='Y'])"/>-->
        <!-- [SP] 2018-08-06 sfb: ***********************************
        Chunking is entirely gone. 
        Now chunk_count will always return zero, which means no chunking will take place. 
        
        -->
        <!--        <xsl:variable name="chunk_count" select="count($topic//section) + count($topic//data[@name='pace_break' and upper-case(@value)='Y'])"/>-->
        <xsl:variable name="chunk_count" select="xs:integer(0)"/>
...
<!--                <xsl:comment>non-chunked topicref:</xsl:comment>-->
                <topicref>
                    <xsl:attribute name="orig_href">
                        <!-- [SP] 2018-05-08 sfb: Point to a different output file if merging.-->
                        <xsl:choose>
                            <xsl:when test="*[contains(@class,' map/topicref ') 
                                ...
                                <xsl:value-of select="replace(@href,'\.dita$','_merge.dita')"/>
                            </xsl:when>
                            <xsl:otherwise>
                                <!-- [SP] 2018-08-06 sfb: NEVERMIND, ALL topics will be _merge. -->
                                <xsl:value-of select="replace(@href,'\.dita$','_merge.dita')"/>
<!--                                <xsl:value-of select="@href"/>                                -->
                            </xsl:otherwise>
                        </xsl:choose>
                    ...
                    <!-- [SP] 2018-05-17 sfb: Drop in the topicsubject for later reltable building.-->
        ...
        <!-- [SP] 2018-05-08 sfb: Locus of nested topicref processing. -->
        <xsl:if test="not(@type='kpe-assessmentOverview')">            
            <!-- [SP] 2018-05-08 sfb: If there are nested topicrefs, created merged version -->
            <xsl:choose>
                <xsl:when test="*[contains(@class,' map/topicref ') 
                    and not(contains(@class,' classify-d/topicsubject ')) 
                    and not(contains(@class,' subjectScheme/subjectdef ')) 
                    ...
                    <xsl:variable name="new_topic" select="replace(@href,'\.dita$','_merge.dita')"/>
                    
                    <xsl:message>Modifying content of <xsl:value-of select="@href"/> into <xsl:value-of select="$new_topic"/>.</xsl:message>
                    <xsl:result-document href="{$new_topic}" method="xml" indent="yes">
                        <xsl:comment>Generated from <xsl:value-of select="@href"/>.</xsl:comment>
                        
                        <xsl:apply-templates select="document(@href)/node()" mode="merge_subtopics">
                            <xsl:with-param name="subtopics" select="*[contains(@class,' map/topicref ') 
                                and not(contains(@class,' classify-d/topicsubject ')) 
                                and not(contains(@class,' subjectScheme/subjectdef ')) 
                                and not(@type='kpe-assessmentOverview')
                                ]" as="element()*"/>
...
<!--                        <xsl:copy-of select="document(@href)/node()"/>                -->
                ...
                <xsl:otherwise>
                    <xsl:message>%%%%%%%%%% NO TOPICS TO MERGE.</xsl:message>
                    <xsl:variable name="new_topic" select="replace(@href,'\.dita$','_merge.dita')"/>
                ...
                <!-- Mimic merge functions, but without merging anything. -->
<!--                    <xsl:apply-templates
    ...
    <!-- [SP] 2018-05-08 sfb: Merge subtopics into current topic. -->
    
    <xsl:template match="*[contains(@class,' topic/topic ')]" mode="merge_subtopics">
        <xsl:param name="subtopics" as="element()*"/>
        
        <xsl:message>XYZZY in merge_subtopics. Count of subtopics is <xsl:value-of select="count($subtopics)"/>.</xsl:message>
        
        <xsl:copy>
            <xsl:apply-templates select="@*|node()" mode="merge_subtopics">
                <xsl:with-param name="subtopics" select="$subtopics" as="element()*"/>
            </xsl:apply-templates>
        </xsl:copy>
    </xsl:template>
    
    <xsl:template match="*[contains(@class,' topic/body ')]" mode="merge_subtopics">
        <xsl:param name="subtopics" as="element()*"/>
        <xsl:copy>
            <xsl:apply-templates select="@*|node()" mode="merge_subtopics"/>
            
            <!-- Now do the merge. --> 
            <xsl:comment>Merge in subtopics.</xsl:comment>
            <xsl:call-template name="merge_topicref">
                <xsl:with-param name="subtopics" select="$subtopics" as="element()*"/>
            </xsl:call-template>
        </xsl:copy>
    </xsl:template>
    ...
    <xsl:template match="@*|node()" mode="merge_subtopics merge_topicref">
        <xsl:copy>
            <xsl:apply-templates select="@*|node()" mode="merge_subtopics"/>            
        </xsl:copy>
    </xsl:template>
    ...
    <xsl:template match="processing-instruction()" mode="merge_subtopics" priority="10">
        <xsl:param name="subtopics" as="element()*"/>
        <xsl:copy/>
    </xsl:template>
    
    <xsl:template name="merge_topicref">
        <xsl:param name="subtopics" as="element()*"/>
        
        <!-- Dereference and process first subtopic. -->
        <xsl:apply-templates select="document($subtopics[1]/@href)/*[contains(@class,' topic/topic ')]" mode="merge_topicref"/>
        
        <xsl:if test="count($subtopics) &gt; 1">
            <xsl:call-template name="merge_topicref">
                <xsl:with-param name="subtopics" select="$subtopics[position() &gt; 1]" as="element()*"/>
            </xsl:call-template>
        </xsl:if>
        
    </xsl:template>
    
    <xsl:template match="*[contains(@class,' topic/topic ')]" mode="merge_topicref">
        <h3>
            <xsl:apply-templates select="*[contains(@class,' topic/title ')]/node()" mode="merge_topicref"/>
        </h3>
        <xsl:apply-templates select="*[contains(@class,' topic/body ')]/node()" mode="merge_topicref"/>
    </xsl:template>
    
    <xsl:template match="*[contains(@class,' topic/section ')]" mode="merge_topicref">
        <xsl:if test="*[contains(@class,' topic/title ')]">
            <h4>
                <xsl:apply-templates select="*[contains(@class,' topic/title ')]/node()" mode="merge_topicref"/>
            </h4>            
        </xsl:if>
        <xsl:apply-templates select="*[not(contains(@class,' topic/title '))]" mode="merge_topicref"/>
    </xsl:template>
    
    <!-- [SP] 2018-05-22 sfb: Eliminate sectiondiv, which is used for conrefs (and seems to mess up the CMS). -->
    <xsl:template match="*[contains(@class,' topic/sectiondiv ')]" mode="merge_topicref">
        <xsl:apply-templates mode="merge_topicref"/>
    </xsl:template>
