...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="bookmap">
        ...
        <xsl:result-document href="shared/launchpage.html" method="html" standalone="yes">
            <xsl:text>&#10;</xsl:text>
            <xsl:text disable-output-escaping="yes">&lt;!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd"></xsl:text><xsl:text disable-output-escaping="yes">
...
<head>
    ...
    /*************************************/]]></xsl:text>
            ...
            var pageArray = new Array(<xsl:value-of select="$array_size"/>); <xsl:apply-templates
                select="//topicref[not(contains(@class, ' bookmap/chapter ')) and contains(@type, 'kpe-assessmentOverview') or contains(@type, 'kpe-concept') or contains(@type, 'kpe-task')]"
                mode="create_index"/>
...
<body onload="doStart(false);" onbeforeunload="doUnload(false);" onunload="doUnload();">
	...
	<div id="headerDiv">
		...
		<div class="coursetitle-ui" style="">
			<h1 id="coursetitle"></h1>
		</div>
	</div>
	
	<div id="column1">
		<iframe name="tocFrame" id="tocFrame" src="toc.html"></iframe>
	</div>
	
	<div id="column2">
		<iframe name="contentFrame" id="contentFrame" src="" allowfullscreen="true" webkitallowfullscreen="true" mozallowfullscreen="true"></iframe>
	</div>

	<div id="navDiv">
		<input type="button" value="&lsaquo; Previous" id="butPrevious" onclick="doPrevious();"/>
		<input type="button" value="Next &rsaquo;" id="butNext" onclick="doNext();"/>
	</div>
	...
	<script type="text/javascript">
		...
		try {
			...
			xmlFile.open("GET", '../imsmanifest.xml', true);
		}
		catch(err) {
		...
		xmlFile.onreadystatechange = function() {
			if(xmlFile.readyState == 4) {
				var node = xmlFile.responseXML.getElementsByTagName('title');
				var ct = node[0].childNodes[0].nodeValue;
				document.getElementsByTagName('h1')[0].innerText = ct;
			}
		}
		xmlFile.send(null);
	</script>
    ...
    <xsl:template
        match="//topicref[not(contains(@class, ' bookmap/chapter ')) and contains(@type, 'kpe-assessmentOverview') or contains(@type, 'kpe-concept') or contains(@type, 'kpe-task')]"
        mode="create_index">

        <xsl:variable name="ancestor_loc"
            select="count(ancestor::topicref[not(contains(@class, ' bookmap/chapter ')) and contains(@type, 'kpe-assessmentOverview') or contains(@type, 'kpe-concept') or contains(@type, 'kpe-task')])"/>
        ...
        <xsl:variable name="ohref_loc">
            ...
            <xsl:choose>
                ...
                <xsl:when test="@type = 'kpe-assessmentOverview'">
                    <xsl:variable name="assessment_count"
                        select="count(preceding::*[contains(@type, 'kpe-assessmentOverview')]) + 1"/>
                    <xsl:variable name="assessment_loc"
                        select="concat('questions', $assessment_count, '.js')"/>
                    <xsl:variable name="assessment_id" select="@id"/>
                    <xsl:variable name="real_assessment"
                        select="//kpe-assessmentOverview[@id = $assessment_id]"/>
                    <xsl:variable name="amp" select="'&#x026;'"/>
                    <xsl:variable name="minimumPassScore">
                        <xsl:choose>
                            <xsl:when
                                test="$real_assessment/prolog/metadata/minimumPassScore/@value != ''">
                                <xsl:value-of
                                    select="$real_assessment/prolog/metadata/minimumPassScore/@value"
                                />
                            </xsl:when>
                    ...
                    <xsl:variable name="assessment_type"
                        select="$real_assessment/prolog/metadata/lmsCategory/@value"/>
                    <xsl:message>minimumPassScore is <xsl:value-of select="$minimumPassScore"
                        /></xsl:message>
                    <xsl:text>shared/assessmenttemplate.html?questions=</xsl:text><xsl:value-of
                        select="$assessment_count"/>&amp;passrate=<xsl:value-of
                        select="$minimumPassScore"/>&amp;assessmenttype=<xsl:value-of
