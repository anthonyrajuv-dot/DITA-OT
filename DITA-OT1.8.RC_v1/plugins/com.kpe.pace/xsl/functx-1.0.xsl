...
-->
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:functx="http://www.functx.com" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:local="http://www.datypic.com/local" xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:dxmlf="http://www.datypic.com/xmlf" exclude-result-prefixes="dxmlf xs" version="2.0">
...
<!--
 ...
 @author  Anthony Raju, Kaplan 
 @version 1.0
 @param   $arg the word or phrase to capitalize 
-->
	<xsl:function name="functx:title-case" as="xs:string?">
		<xsl:param name="arg" as="xs:string?"/>
		<xsl:sequence select="     concat(upper-case(substring($arg,1,1)),lower-case(substring($arg,2)) )  "/>
	</xsl:function>
	
	
<!--
  Returns the Capitalised first three character of a month string 

 @author  Anthony Raju, Kaplan 
 @version 1.0
 @param   $arg the word or phrase to capitalize 
-->
	<xsl:function name="functx:first-three-let-month" as="xs:string?">
		<xsl:param name="arg" as="xs:string?"/>
		<xsl:sequence select="     upper-case(substring($arg,1,3))  "/>
	</xsl:function>


<!--
  Returns the last two character of a year string 

 @author  Anthony Raju, Kaplan 
 @version 1.0
 @param   $arg the word or phrase to capitalize 
-->
	<xsl:function name="functx:last-two-num-year" as="xs:string?">
		<xsl:param name="arg" as="xs:string?"/>
		<xsl:sequence select="     (substring($arg,3))  "/>
	</xsl:function>

<!--
  Returns Two Letter Abbreviate US State String 

 @author  Anthony Raju, Kaplan 
 @version 1.0
 @param   $arg the word or phrase to abbreviate 
-->
	<xsl:function name="functx:abbr-UsState" as="xs:string?">
		<xsl:param name="arg" as="xs:string?"/>
	<xsl:choose>
		<xsl:when test="normalize-space($arg) = 'Alabama' "><xsl:sequence select=" 'AL' "/></xsl:when>
		<xsl:when test="normalize-space($arg) = 'Alaska' "><xsl:sequence select=" 'AK' "/></xsl:when>
		<xsl:when test="normalize-space($arg) = 'Arizona' "><xsl:sequence select=" 'AZ' "/></xsl:when>
		<xsl:when test="normalize-space($arg) = 'Arkansas' "><xsl:sequence select=" 'AR' "/></xsl:when>
		<xsl:when test="normalize-space($arg) = 'California' "><xsl:sequence select=" 'CA' "/></xsl:when>
		<xsl:when test="normalize-space($arg) = 'Colorado' "><xsl:sequence select=" 'CO' "/></xsl:when>
		<xsl:when test="normalize-space($arg) = 'Connecticut' "><xsl:sequence select=" 'CT' "/></xsl:when>
		<xsl:when test="normalize-space($arg) = 'Delaware' "><xsl:sequence select=" 'DE' "/></xsl:when>
		<xsl:when test="normalize-space($arg) = 'Florida' "><xsl:sequence select=" 'FL' "/></xsl:when>
		<xsl:when test="normalize-space($arg) = 'Georgia' "><xsl:sequence select=" 'GA' "/></xsl:when>
		<xsl:when test="normalize-space($arg) = 'Hawaii' "><xsl:sequence select=" 'HI' "/></xsl:when>
		<xsl:when test="normalize-space($arg) = 'Idaho' "><xsl:sequence select=" 'ID' "/></xsl:when>
		<xsl:when test="normalize-space($arg) = 'Illinois' "><xsl:sequence select=" 'IL' "/></xsl:when>
		<xsl:when test="normalize-space($arg) = 'Indiana' "><xsl:sequence select=" 'IN' "/></xsl:when>
		<xsl:when test="normalize-space($arg) = 'Iowa' "><xsl:sequence select=" 'IA' "/></xsl:when>
		<xsl:when test="normalize-space($arg) = 'Kansas' "><xsl:sequence select=" 'KS' "/></xsl:when>
		<xsl:when test="normalize-space($arg) = 'Kentucky' "><xsl:sequence select=" 'KY' "/></xsl:when>
		<xsl:when test="normalize-space($arg) = 'Louisiana' "><xsl:sequence select=" 'LA' "/></xsl:when>
		<xsl:when test="normalize-space($arg) = 'Maine' "><xsl:sequence select=" 'ME' "/></xsl:when>
		<xsl:when test="normalize-space($arg) = 'Maryland' "><xsl:sequence select=" 'MD' "/></xsl:when>
		<xsl:when test="normalize-space($arg) = 'Massachusetts' "><xsl:sequence select=" 'MA' "/></xsl:when>
		<xsl:when test="normalize-space($arg) = 'Michigan' "><xsl:sequence select=" 'MI' "/></xsl:when>
		<xsl:when test="normalize-space($arg) = 'Minnesota' "><xsl:sequence select=" 'MN' "/></xsl:when>
		<xsl:when test="normalize-space($arg) = 'Mississippi' "><xsl:sequence select=" 'MS' "/></xsl:when>
		<xsl:when test="normalize-space($arg) = 'Missouri' "><xsl:sequence select=" 'MO' "/></xsl:when>
		<xsl:when test="normalize-space($arg) = 'Montana' "><xsl:sequence select=" 'MT' "/></xsl:when>
		<xsl:when test="normalize-space($arg) = 'Nebraska' "><xsl:sequence select=" 'NE' "/></xsl:when>
		<xsl:when test="normalize-space($arg) = 'Nevada' "><xsl:sequence select=" 'NV' "/></xsl:when>
		<xsl:when test="normalize-space($arg) = 'New Hampshire' "><xsl:sequence select=" 'NH' "/></xsl:when>
		<xsl:when test="normalize-space($arg) = 'New Jersey' "><xsl:sequence select=" 'NJ' "/></xsl:when>
		<xsl:when test="normalize-space($arg) = 'New Mexico' "><xsl:sequence select=" 'NM' "/></xsl:when>
		<xsl:when test="normalize-space($arg) = 'New York' "><xsl:sequence select=" 'NY' "/></xsl:when>
		<xsl:when test="normalize-space($arg) = 'North Carolina' "><xsl:sequence select=" 'NC' "/></xsl:when>
		<xsl:when test="normalize-space($arg) = 'North Dakota' "><xsl:sequence select=" 'ND' "/></xsl:when>
		<xsl:when test="normalize-space($arg) = 'Ohio' "><xsl:sequence select=" 'OH' "/></xsl:when>
		<xsl:when test="normalize-space($arg) = 'Oklahoma' "><xsl:sequence select=" 'OK' "/></xsl:when>
		<xsl:when test="normalize-space($arg) = 'Oregon' "><xsl:sequence select=" 'OR' "/></xsl:when>
		<xsl:when test="normalize-space($arg) = 'Pennsylvania' "><xsl:sequence select=" 'PA' "/></xsl:when>
		<xsl:when test="normalize-space($arg) = 'Rhode Island' "><xsl:sequence select=" 'RI' "/></xsl:when>
		<xsl:when test="normalize-space($arg) = 'South Carolina' "><xsl:sequence select=" 'SC' "/></xsl:when>
		<xsl:when test="normalize-space($arg) = 'South Dakota' "><xsl:sequence select=" 'SD' "/></xsl:when>
		<xsl:when test="normalize-space($arg) = 'Tennessee' "><xsl:sequence select=" 'TN' "/></xsl:when>
		<xsl:when test="normalize-space($arg) = 'Texas' "><xsl:sequence select=" 'TX' "/></xsl:when>
		<xsl:when test="normalize-space($arg) = 'Utah' "><xsl:sequence select=" 'UT' "/></xsl:when>
		<xsl:when test="normalize-space($arg) = 'Vermont' "><xsl:sequence select=" 'VT' "/></xsl:when>
		<xsl:when test="normalize-space($arg) = 'Virginia' "><xsl:sequence select=" 'VA' "/></xsl:when>
		<xsl:when test="normalize-space($arg) = 'Washington' "><xsl:sequence select=" 'WA' "/></xsl:when>
		<xsl:when test="normalize-space($arg) = 'West Virginia' "><xsl:sequence select=" 'WV' "/></xsl:when>
		<xsl:when test="normalize-space($arg) = 'Wisconsin' "><xsl:sequence select=" 'WI' "/></xsl:when>
		<xsl:when test="normalize-space($arg) = 'Wyoming' "><xsl:sequence select=" 'WY' "/></xsl:when>
		<xsl:when test="normalize-space($arg) = 'MX' "><xsl:sequence select=" 'MX' "/></xsl:when>
		<xsl:when test="normalize-space($arg) = 'MS MEE' "><xsl:sequence select=" 'MM' "/></xsl:when>
		<xsl:otherwise>
			<xsl:sequence select="normalize-space($arg)"/>
		</xsl:otherwise>
	</xsl:choose>
...
<!--
Adds zeros to the beginning of number 

 @author  Anthony Raju, Kaplan 
 @version 1.0
 @param   $arg the input number 
-->
	<xsl:function name="functx:addZeroToQ-num" as="xs:anyAtomicType*">
	<xsl:param name="arg" as="xs:integer?"/>
		<xsl:choose>
			<xsl:when test="$arg &lt; 10 ">
				<xsl:sequence select="concat('00',string($arg))"/>
			</xsl:when>
			<xsl:when test="$arg &gt;= 10 and $arg &lt; 100 ">
				<xsl:sequence select="concat('0',string($arg))"/>
			</xsl:when>
		</xsl:choose>
	</xsl:function>
...
<!--
  Return questionName of a string (Bar Essay) 

 @author  Anthony Raju, Kaplan 
 @version 1.0
 @param   $arg1 $arg2 $arg3 $arg4 the word or phrase to abbreviate
-->
	<xsl:function name="functx:questionName" as="xs:anyAtomicType*">
		<xsl:param name="arg1" as="xs:string?"/>
		<xsl:param name="arg2" as="xs:string?"/>
		<xsl:param name="arg3" as="xs:string?"/>
		<xsl:param name="arg4" as="xs:integer?"/>
		<xsl:param name="arg5" as="xs:string?"/>
		<xsl:choose>
			<xsl:when test="$arg5 !='' ">
				<xsl:sequence select="normalize-space($arg5)"/>
			</xsl:when>
			<xsl:when test="$arg1 ='' and $arg2 ='' and $arg3 ='' and $arg4 = 0 ">
				<xsl:sequence select="''"/>
			</xsl:when>
			<xsl:otherwise>
				<xsl:sequence select=" functx:temp-getQuestionName(concat( functx:abbr-UsState($arg1),functx:last-two-num-year($arg2),functx:first-three-let-month($arg3),functx:addZeroToQ-num($arg4) ) )  "/>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:function>
...
<!--
  Removes ../ from the @href value and return filename  (Bar Essay) 

 @author  Anthony Raju, Kaplan 
 @version 1.0
 @param   $arg1
-->
	<xsl:function name="functx:remove-Leading-Dots" as="xs:anyAtomicType*">
		<xsl:param name="arg1" as="xs:string?"/>
		<xsl:choose>
			<xsl:when test="starts-with($arg1,'../')">
				<xsl:sequence select="functx:remove-Leading-Dots(substring-after($arg1,'../'))"/>
			</xsl:when>
			<xsl:otherwise>
...
<!--
  Returns 1 of 7 Performance Type for BAR Essay Performanance Tracker as String 

 @author  Anthony Raju, Kaplan 
 @version 1.0
 @param   $arg the word or phrase to abbreviate 
-->
	<xsl:function name="functx:pt-typeConv" as="xs:string?">
		<xsl:param name="arg" as="xs:string?"/>
		<xsl:choose>
			<xsl:when test="$arg = 'brief' "><xsl:sequence select=" 'Brief' "/></xsl:when>
			<xsl:when test="$arg = 'diagnostic' "><xsl:sequence select=" 'Diagnostic' "/></xsl:when>
			<xsl:when test="$arg = 'essay_bank' "><xsl:sequence select=" 'Persuasive Memo' "/></xsl:when>
			<xsl:when test="$arg = 'letter' "><xsl:sequence select=" 'Objective Letter' "/></xsl:when>
			<xsl:when test="$arg = 'memo' "><xsl:sequence select=" 'Objective Memo' "/></xsl:when>
			<xsl:when test="$arg = 'other' "><xsl:sequence select=" 'Other' "/></xsl:when>
			<xsl:when test="$arg = 'spe' "><xsl:sequence select=" 'Persuasive Letter' "/></xsl:when>
			<xsl:otherwise>
...
<!--
  Returns question type for GMAT 

 @author  Anthony Raju, Kaplan 
 @version 1.0
 @param   $arg is the element-name to get check for relevant question type 
-->
	<xsl:function name="functx:get-QType" as="xs:string?">
		<xsl:param name="arg" as="xs:string?"/>
		<xsl:choose>
			<xsl:when test="$arg = 'lcGraphicsInterpretation' "><xsl:sequence select=" 'lcGraphicsInterpretation' "/></xsl:when>
			<xsl:when test="$arg = 'lcReadingComprehension' "><xsl:sequence select=" 'lcReadingComprehension' "/></xsl:when>
			<xsl:when test="$arg = 'lcDataSufficiency' "><xsl:sequence select=" 'lcDataSufficiency' "/></xsl:when>
			<xsl:when test="$arg = 'lcCriticalReasoning' "><xsl:sequence select=" 'lcCriticalReasoning' "/></xsl:when>
			<xsl:when test="$arg = 'lcMultiSourceReasoning' "><xsl:sequence select=" 'lcMultiSourceReasoning' "/></xsl:when>
			<xsl:when test="$arg = 'lcProblemSolving' "><xsl:sequence select=" 'lcProblemSolving' "/></xsl:when>
			<xsl:when test="$arg = 'lcTwoPartAnalysis' "><xsl:sequence select=" 'Two-Part Analysis' "/></xsl:when>
			<xsl:when test="$arg = 'lcTableAnalysis' "><xsl:sequence select=" 'lcTableAnalysis' "/></xsl:when>
			<xsl:when test="$arg = 'lcMultiSourceReasoningMultiple' "><xsl:sequence select=" 'lcMultiSourceReasoningMultiple' "/></xsl:when>
			<xsl:otherwise>
				<xsl:sequence select="$arg"/>
			</xsl:otherwise>
		</xsl:choose>
...
<!--
  Temporary	
  Returns => (INDIANLAW001|INDIANLAW002|INDIANLAW003|INDIANLAW004|INDIANLAW005|WIESSAY001|WIESSAY002|WIESSAY003|WIESSAY004|WIESSAY005) 

 @author  Anthony Raju, Kaplan 
 @version 1.0
 @param   $arg the word or phrase matched
-->
	<xsl:function name="functx:temp-getQuestionName" as="xs:string?">
		<xsl:param name="arg" as="xs:string?"/>
		<xsl:choose>
			<xsl:when test="$arg = 'SD50FEB001' "><xsl:sequence select=" 'INDIANLAW001' "/></xsl:when>
			<xsl:when test="$arg = 'SD50FEB002' "><xsl:sequence select=" 'INDIANLAW002' "/></xsl:when>
			<xsl:when test="$arg = 'SD50FEB003' "><xsl:sequence select=" 'INDIANLAW003' "/></xsl:when>
			<xsl:when test="$arg = 'SD50FEB004' "><xsl:sequence select=" 'INDIANLAW004' "/></xsl:when>
			<xsl:when test="$arg = 'SD50FEB005' "><xsl:sequence select=" 'INDIANLAW005' "/></xsl:when>
			<xsl:when test="$arg = 'WI50FEB001' "><xsl:sequence select=" 'WIESSAY001' "/></xsl:when>
			<xsl:when test="$arg = 'WI50FEB002' "><xsl:sequence select=" 'WIESSAY002' "/></xsl:when>
			<xsl:when test="$arg = 'WI50FEB003' "><xsl:sequence select=" 'WIESSAY003' "/></xsl:when>
			<xsl:when test="$arg = 'WI50FEB004' "><xsl:sequence select=" 'WIESSAY004' "/></xsl:when>
			<xsl:when test="$arg = 'WI50FEB005' "><xsl:sequence select=" 'WIESSAY005' "/></xsl:when>
			<xsl:otherwise>
				<xsl:sequence select="$arg"/>
			</xsl:otherwise>
		</xsl:choose>
