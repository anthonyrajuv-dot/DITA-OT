    ...
    <xsl:template match="supermap">
        <map>
            ...
            <xsl:apply-templates select="los_summary"/>
        	   ...
        	   <!-- Handle Prompts. -->
            <xsl:apply-templates select="prompts_summary"/>
...
<!--                    <xsl:for-each select="following-sibling::examMap[@assessment_type!='test-exam-primary']">-->
    ...
    <xsl:template name="get_file_id_base">
    	...
    	<xsl:message>ARV Test here: <xsl:value-of select="$preceding_count"></xsl:value-of></xsl:message>
    	<xsl:message>file_id_base: <xsl:value-of select="$file_id_base"></xsl:value-of></xsl:message>
    	
    	   <xsl:choose>
    	   	<xsl:when test="($offering_type != 'essay') and ($offering_type != 'nexGen') and $preceding_count &gt; 0">
                <xsl:variable name="number">
                    <xsl:number value="$preceding_count" format="a"/>
                </xsl:variable>
                <xsl:value-of select="concat($file_id_base,'R',$number)"/>
            </xsl:when>
    	   	<xsl:when test="(($offering_type = 'essay') or ($offering_type = 'nexGen')) and $preceding_count &gt; 0">
    	   		<xsl:value-of select="concat('SAME_ESSAY_',$file_id_base)"/>
    	   	</xsl:when>
            <xsl:otherwise>
...
<!--        <xsl:variable name="los_number_zi">
	...
	<xsl:template match="prompts_summary">
	...
	<xsl:template match="prompts">
		<!-- Create the related files. -->
		<xsl:variable name="topic" select="document(@orig_href)/*" as="element()*"/>
		<xsl:variable name="prompts_number" select="functx:index-of-node($prompts_summary/*,.)"/>
		
		<!-- Get the keys attribute before finding unit. -->
		<xsl:variable name="keys" select="@keys"/>
		
		<!-- Find the first topicprompts in which the keys appears. This will give the topichead for the unit. -->
		<xsl:variable name="topicprompt" select="//topicprompt[@keyref = $keys]"/>
		<xsl:variable name="topichead" select="$topicprompt[1]/ancestor::topichead" as="element()*"/>
		<xsl:variable name="unit" select="$topichead/@unit"/>
		
		<!-- Within the topichead, gather all topicprompt elements. -->
		<!--        <xsl:variable name="all_topicprompts" select="$topicprompt//topicprompt" as="element()*"/>-->
		<xsl:variable name="all_topicprompts" as="element()*">
			<topicprompt>
				<xsl:for-each select="$topichead//topicprompt">
					<xsl:copy>
						<xsl:apply-templates select="@*" mode="identity"/>                    
					</xsl:copy>
				</xsl:for-each>
		...
		<!--        <xsl:message>Found <xsl:value-of select="count($all_topicprompts)"/> topicprompts elements. **************************</xsl:message>
        <xsl:message>Found <xsl:value-of select="count($all_topicprompts/topicprompt)"/> topicprompt elements. **************************</xsl:message>
        
        <xsl:message>Position of keyref for <xsl:value-of select="$keys"/> is <xsl:value-of select="count($all_topicprompts/topicprompt[@keyref = $keys][1]/preceding-sibling::topicprompt)"/>. **************************</xsl:message>
-->        
		<!--        <xsl:apply-templates select="$all_topicprompts[@keyref=$keys][1]" mode="dump_topicprompts"/>-->
		
		<!-- Find the @keys topicprompt and count preceding siblings. -->
		<xsl:variable name="unit_prompt_number" select="count($all_topicprompts/topicprompts[@keyref = $keys][1]/preceding-sibling::topicprompt) + 1"/>
		<!--        <xsl:variable name="unit_prompt_number" select="count(topicprompt[1]/preceding-sibling::topicprompt) + 1"/>-->
		
		<xsl:call-template name="new_prompt_master">
			<xsl:with-param name="topic" select="$topic"/>
			<xsl:with-param name="name_base" select="/supermap/@name_base"/>
			<xsl:with-param name="course_num" select="/supermap/@course_num"/>
			<xsl:with-param name="prompts_number" select="$prompts_number"/>
			<!-- Zero-filled unit number. -->
			<xsl:with-param name="unit">
				<xsl:choose>
					<xsl:when test="$unit = 'I.1'">
			...
			<xsl:with-param name="unit_prompt_number" select="$unit_prompt_number"/>
		</xsl:call-template>
	</xsl:template>
	
	<xsl:template match="topicprompt" mode="dump_topicprompts">
		<xsl:message>
			<xsl:value-of select="name()"/> (parent: <xsl:value-of select="name(parent::*)"/>) 
			keyref: <xsl:value-of select="@keyref"/>
			preceding-siblings: <xsl:value-of select="count(preceding-sibling::*)"/>
	...
	<xsl:template name="new_prompt_master">
		<xsl:param name="topic"/>
		<xsl:param name="name_base"/>
		<xsl:param name="course_num"/>
		<xsl:param name="prompts_number"/>
		<xsl:param name="unit"/>
		<xsl:param name="unit_prompt_number"/>
		
		
		<!-- MREP: filename now comes from the original filename. -->
		<!--        <xsl:variable name="file_name" select="concat('los/',$name_base,'_LOG',$los_number,'_LO',$los_number,'.dita')"/>-->
		<xsl:variable name="file_name" select="replace(@orig_href,'.*/([^/]*\.[^/]*)$','prompts/$1')"/>
		
		<xsl:variable name="out_file" select="concat('file:///',translate($OUT_DIR,'\','/'),'/',replace($file_name,'.dita','.xml'))"/>
		<xsl:message>(new_prompt_master) out_file is "<xsl:value-of select="$out_file"/>".</xsl:message>
		
		<!-- Create zero-filled integer forms of the course number and PROMPT number. -->
		<xsl:variable name="course_num_zi">
			<xsl:number value="$course_num" format="00"/>
		</xsl:variable>
		
		<!--  USED as is for PROMPT: need to check and change this comments:      <xsl:variable name="los_number_zi">
            <xsl:number value="$los_number" format="00"/>
        </xsl:variable>-->
		
		<xsl:variable name="unit_prompt_number_zi">
			<xsl:number value="$unit_prompt_number" format="00"/>
		</xsl:variable>
		
		<!-- <xsl:variable name="title" select="concat('LOS ',$course_num_zi,'.',$unit,'.',$los_number_zi)"/> -->
		<xsl:variable name="title" select="concat('PROMPT ',$course_num_zi,'.',$unit,'.',$unit_prompt_number_zi)"/>
		
		<!-- Process the href file into a new file -->
		<xsl:result-document href="{$out_file}" method="xml" indent="yes" doctype-public="''" doctype-system="''">
			
			<!-- All mode="new_topic" templates are defined in new_topic_task.xsl. -->
			<xsl:apply-templates select="$topic" mode="new_prompt_master">
				<xsl:with-param name="title" select="$title"/>
			</xsl:apply-templates>
