...
-->
		...
		<questionItem>
            ...
            <xsl:apply-templates select="topicsubject" mode="find_los">
				...
				<xsl:apply-templates mode="other_meta">
                <xsl:with-param name="q_number" select="$q_number"/>
            </xsl:apply-templates>
...
-->
	...
	<!-- BAR Essay -->
	<xsl:template match="/kpe-question/prolog/metadata" mode="other_meta">
		<xsl:for-each select="othermeta">
			<data name="attributeMap"></data>
		</xsl:for-each>		
	</xsl:template>
    ...
    <xsl:template match="*[contains(@class,' topic/topic ')]" mode="new_learning_question">
		...
		<xsl:choose>
		    <xsl:when test="/kpe-question/contains(@id,'barEssay_section_')">
        ...
        <xsl:variable name="bar_section" select="//*/kpe-questionBody/lcIntro/@outputclass"/>
			...
			<xsl:variable name="mappedValue" select="//*/note/p[last()]/pointValue/@value"/>            
		...
		<itemBody>
			...
			<xsl:choose>
                    <xsl:when test="@outputclass='issue'">
                        <issue>
                                <p><xsl:apply-templates select="node()" mode="identity"/></p>
                        </issue>
                    </xsl:when>
					<xsl:when test="@outputclass='rules'">
                        <rules>
						<xsl:choose>
							<xsl:when test="following-sibling::ph">
				...
				<xsl:when test="@outputclass='analysis'">
					<analysis>
					<xsl:for-each select="ph">
                        <p><xsl:apply-templates select="node()" mode="identity"/></p>
					</xsl:for-each>
					...
					<xsl:when test="@outputclass='conclusion'">
                        <conclusion>
                                <p><xsl:apply-templates select="node()" mode="identity"/></p>
                        </conclusion>
                    </xsl:when>
