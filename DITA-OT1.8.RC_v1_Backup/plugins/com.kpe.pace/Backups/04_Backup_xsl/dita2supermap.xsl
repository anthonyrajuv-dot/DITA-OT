...
-->            
   ...
   <xsl:template match="*[contains(@class,' mapgroup-d/topicgroup ')]">
    ...
    <xsl:template
        ...
        <!-- [SP] 2020-11-30 sfb: When handling nested topicrefs determine if the first is a special case.-->
        <xsl:param name="ignore_first_chunk_or_topic" as="xs:boolean" select="false()"/>
        <xsl:param name="first_topic_id" as="xs:string" select="''"/>
        <xsl:param name="parent_navtitle" as="xs:string" select="''"/>
        <xsl:param name="a-head" tunnel="yes"/>
...
-->
        ...
        <!--MW: Check this section-->
        <xsl:variable name="navtitle">
            <xsl:choose>
                <xsl:when test="empty($parent_navtitle) or $parent_navtitle = ''">
                    <xsl:call-template name="get_navtitle"/>                    
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of select="$parent_navtitle"/>
                    <xsl:if test="not(contains($parent_navtitle,' (cont.)'))">
...
<!--                <xsl:when test="$topic//section[title] or $topic//data[@name='pace_break' and upper-case(@value)='Y']">
        ...
        <!-- Need to know if this is a parent element that should go straight into the first child. -->
        <xsl:variable name="body_elements" select="count($topic/*[contains(@class,' topic/body ')]/*)"/>
        ...
        <xsl:choose>
            <!-- [SP] 2020-11-30 sfb: Handle A-head to B-head-->
            <xsl:when test="$body_elements = 0 and $task_category = 'kpe-concept'">
                <!-- Save the title/navtitle (for the A-head) -->
                <xsl:variable name="a-head">
                    <xsl:apply-templates select="$topic/*[contains(@class,' topic/title ')]"/>
                </xsl:variable>
                
<!--                <xsl:message>%%%%%%%%%%%%%% $body_elements = 0</xsl:message>
                <xsl:message>a-head is <xsl:value-of select="$a-head"/>.</xsl:message>-->
                
                <!-- Get the first topicref of the children.-->
                <xsl:variable name="first_child" select="*[contains(@class,' map/topicref ')][1]"/>
                <xsl:variable name="first_child_topic" select="document($first_child/@href)/*" as="element()*"/>
                
                <!-- Get the title/navtitle for the B-head -->
                <xsl:variable name="b-head">
                    <xsl:apply-templates select="$first_child_topic/*[contains(@class,' topic/title ')]"/>
                </xsl:variable>
<!--                <xsl:message>b-head is <xsl:value-of select="$b-head"/>.</xsl:message>
-->                
                <!-- Test chunking -->
                <xsl:variable name="first_child_chunk_count" 
                    select="count($first_child_topic//section) + count($first_child_topic//data[@name='pace_break' and upper-case(@value)='Y'])"/>
                <xsl:message>first_child_chunk_count is: <xsl:value-of select="$first_child_chunk_count"/>.</xsl:message>
                
                <!-- Build chunkmap, if necessary (initially assuming we do). -->
                <!-- This might be overkill...if the first_child_chunk_count is non-zero, just grab the first section or
                    chunk (pace_break) and use it as content. -->
                <xsl:message>Creating first_child_chunk_map.</xsl:message>
                <xsl:variable name="first_child_chunk_map" as="element()*">
                    <xsl:apply-templates select="$first_child_topic/descendant-or-self::*[contains(@class,' topic/body ')]" mode="chunk_map"/>
                </xsl:variable>
                
                <xsl:message>first_child_chunk_map contains <xsl:value-of select="count($first_child_chunk_map/descendant::*)"/> elements.</xsl:message>
                
                <!-- Subsequent code pulled from chunk iterator -->
                <!-- href of the actual current topicref -->
                <xsl:variable name="parent_href" select="@href"/>
                <xsl:variable name="child_href" select="$first_child/@href"/>
                
                <!-- Get the path portion of the href. -->
                <xsl:variable name="parent_file_path" select="replace($parent_href,'^(.*)/[^/]*$','$1')"/>
                <!-- Get the filename potion of the href, minus the extension. -->
                <xsl:variable name="parent_base_filename" select="replace($parent_href,'^.*/([^/]*).dita$','$1')"/>
                
                <xsl:message>  parent_file_path is <xsl:value-of select="$parent_file_path"/>.</xsl:message>
                <xsl:message>  parent_base_filename is <xsl:value-of select="$parent_base_filename"/>.</xsl:message>
                
                <!-- We know this is the first chunk, so we don't have to bother with creating a sequence letter. -->
                <xsl:variable name="parent_filename" select="concat($parent_file_path,'/',$parent_base_filename,'_a.dita')"/>
                <xsl:message> parent_filename is <xsl:value-of select="$parent_filename"/>.</xsl:message>

                
                <xsl:variable name="parent_topic_id" select="concat($parent_base_filename,'_a')"/>
                <xsl:message>  parent_topic_id is <xsl:value-of select="$parent_topic_id"/>.</xsl:message>



                <!-- DO THE SAME THING FOR THE CHILD -->
                <!-- MIGHT BE ABLE TO ELIMINATE THE PARENT VARS -->
                <!-- Get the path portion of the href. -->
                <xsl:variable name="child_file_path" select="replace($child_href,'^(.*)/[^/]*$','$1')"/>
                <!-- Get the filename potion of the href, minus the extension. -->
                <xsl:variable name="child_base_filename" select="replace($child_href,'^.*/([^/]*).dita$','$1')"/>
                
                <xsl:message>  child_file_path is <xsl:value-of select="$child_file_path"/>.</xsl:message>
                <xsl:message>  child_base_filename is <xsl:value-of select="$child_base_filename"/>.</xsl:message>
                
                <!-- We know this is the first chunk, so we don't have to bother with creating a sequence letter. -->
                <xsl:variable name="child_filename" select="concat($child_file_path,'/',$child_base_filename,'_a.dita')"/>
                <xsl:message> child_filename is <xsl:value-of select="$child_filename"/>.</xsl:message>
                
                
                <xsl:variable name="child_topic_id" select="concat($child_base_filename,'_a')"/>
                <xsl:message>  child_topic_id is <xsl:value-of select="$child_topic_id"/>.</xsl:message>
                
                <xsl:variable name="first_topic_chunk" as="element()*">
                    <!-- Passes in the entire topic. We then have to find the chunk and output only that portion. -->
                    <xsl:apply-templates select="$first_child_topic" mode="get_first_chunk">
                        <xsl:with-param name="chunk_id" select="$first_child_chunk_map/@id"/>
                        <xsl:with-param name="counter" select="1"/>
                        <xsl:with-param name="chunk_map" select="$first_child_chunk_map"/>
                        <xsl:with-param name="topic_id" select="$child_topic_id"/>
                        <xsl:with-param name="a-head" select="$a-head" tunnel="yes"/>
                        <xsl:with-param name="b-head" select="$b-head" tunnel="yes"/>
                        <xsl:with-param name="navtitle" select="$navtitle" tunnel="yes"/>
                    </xsl:apply-templates>
                    
                </xsl:variable>
                
                <!-- Generate the new file. -->
                <xsl:result-document href="{$child_filename}" 
                    ...
                    <xsl:copy-of select="$first_topic_chunk"/>
                </xsl:result-document>
                
                <!-- Count the number of <fn> elements. -->
                <xsl:variable name="first_child_fn_count" select="count($first_topic_chunk/descendant-or-self::*[contains(@class,' topic/fn ')])"/>
                
                <!-- Create the topicref. -->
                <topicref>
                    <xsl:attribute name="orig_href">
                        <xsl:value-of select="$child_filename"/>
                    </xsl:attribute>
                    <!-- Set the task_category. If it's not a task, find another related value. -->
                    <xsl:attribute name="task_category">
                    ...
                    <xsl:attribute name="fn_count">
                        <xsl:value-of select="$first_child_fn_count"/>
                    </xsl:attribute>
                    
                    <xsl:if test="$has_chunking">
                        <xsl:call-template name="chunk_map_iterator">
                            <xsl:with-param name="topic" select="$first_child_topic"/>
                            <xsl:with-param name="chunk_map" select="$first_child_chunk_map"/>
                            ...
                            <xsl:with-param name="counter" select="2" as="xs:integer"/>
                            <xsl:with-param name="a-head" select="$a-head" tunnel="yes"/>
                            <xsl:with-param name="b-head" select="$b-head" tunnel="yes"/>
                            <xsl:with-param name="child_href" select="$child_href" tunnel="yes"/>
                        </xsl:call-template>
                    </xsl:if>
                    
                    <!-- [SP] 2020-12-08 sfb: Process the rest of the child topicrefs. -->
                    <xsl:apply-templates select="*[contains(@class,' map/topicref ') and not(contains(@class,' subjectScheme/subjectdef '))][position() gt 1]">
                        <xsl:with-param name="parent_navtitle" select="$navtitle"/>
                        <xsl:with-param name="a-head" select="$a-head" tunnel="yes"/>
                    </xsl:apply-templates>
...
<!--                <xsl:comment>sections that contain pace breaks: <xsl:value-of select="count($topic//section/data[@name='pace_break' and upper-case(@value)='Y'])"/>.</xsl:comment>-->
                ...
                <!-- Build a temporary structure (the chunk_map) that maps the location of each chunk in the topic. -->
...
<!--                <xsl:message>chunk_map contains <xsl:value-of select="count($chunk_map/descendant::*)"/> elements.</xsl:message>-->
                <xsl:call-template name="chunk_map_iterator">
                    ...
                    <xsl:with-param name="task_category_attr" select="$task_category_attr"/>
                    <xsl:with-param name="type_attr" select="$type_attr"/>
                    <xsl:with-param name="a-head" select="$navtitle" tunnel="yes"/>
                    <xsl:with-param name="navtitle" select="$navtitle"/>
...
<!--                <xsl:comment>non-chunked topicref:</xsl:comment>-->
               ...
               <!-- MW: Check here-->
                <topicref>
                    ...
                    <xsl:attribute name="chunking">
                        <xsl:choose>
                            <xsl:when test="parent::topicref">
                                <xsl:value-of select="$has_chunking"/>
                            </xsl:when>
                            <xsl:otherwise>
                                <xsl:value-of select="true()"/>
                            </xsl:otherwise>
...
<!--                    <xsl:message>type "<xsl:value-of select="$type_attr"/>" for href: <xsl:value-of select="@href"/>.</xsl:message>-->
                    ...
                    <!-- [SP] 2020-11-16 sfb: Handle nested topicrefs. -->
                ...
                <xsl:apply-templates
                    ...
                    <!-- [SP] 2020-11-30 sfb: Pass flag in that indicates that first topic or first chunk in first topic has been used. -->
                    <xsl:with-param name="ignore_first_chunk_or_topic">
                        <!-- IF $body_elements = 0, set to true. -->
                        <xsl:value-of select="false()"/>
                    </xsl:with-param>
        ...
        <xsl:if test="$has_chunking = true() and not(@type='kpe-assessmentOverview')">
            <xsl:apply-templates
                select="*[contains(@class,' map/topicref ') 
                and not(contains(@class,' classify-d/topicsubject ')) 
                and not(contains(@class,' subjectScheme/subjectdef ')) 
    ...
    <xsl:template name="chunk_map_iterator">
        ...
        <xsl:param name="a-head" tunnel="yes"/>
        <xsl:param name="b-head" tunnel="yes"/>
        <xsl:param name="child_href" tunnel="yes"/>
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                ...
                <xsl:variable name="href">
                    <xsl:choose>
                        <xsl:when test="empty($child_href) or $child_href = ''">
                            <xsl:value-of select="@href"/>
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:value-of select="$child_href"/>
                        </xsl:otherwise>
                    </xsl:choose>
                </xsl:variable>
                <xsl:message>(chunk_map_iterator) $href is <xsl:value-of select="$href"/></xsl:message>
                <xsl:message>($a-head is <xsl:value-of select="$a-head"/>)</xsl:message>
                    ...
                    <xsl:if test="not(empty($navtitle))">
                        <navtitle>
                            <xsl:choose>
                                ...
                                <xsl:otherwise>
                                    <xsl:copy-of select="$navtitle"/>
                                    <xsl:if test="not(contains($navtitle,'(cont.)'))">
...
<!--            <xsl:apply-templates
    ...
    <!-- CHUNKING IDENTITY TRANSFORMS -->
    <xsl:template match="*[contains(@class,' topic/topic ')]" mode="chunk_topic">
        <xsl:param name="chunk_id"/>
        <xsl:param name="counter"/>
        <xsl:param name="chunk_map" as="element()*"/>
        <xsl:param name="topic_id"/>
        ...
        <xsl:copy>
            ...
            <xsl:choose>
                ...
                <xsl:when test="not(empty($a-head))">
                    <xsl:apply-templates select="node()" mode="chunk_topic">
                        <xsl:with-param name="chunk_id" select="$chunk_id"/>
                        <xsl:with-param name="counter" select="$counter"/>
                        <xsl:with-param name="chunk_map" select="$chunk_map"/>
                        <xsl:with-param name="topic_id" select="$topic_id"/>
                        <xsl:with-param name="a-head" select="$a-head" tunnel="yes"/>
                        <xsl:with-param name="b-head" tunnel="yes">
    ...
    <!-- [SP] 2020-12-07 sfb: Special case for empty topic content (a-head)-->
    <!-- Get first (and only the first) chunk.
         Defining content for a variable,    
     -->
    <xsl:template match="*[contains(@class,' topic/topic ')]" mode="get_first_chunk">
        <xsl:param name="chunk_id"/>
        <xsl:param name="counter"/>
        <xsl:param name="chunk_map" as="element()*"/>
        <xsl:param name="topic_id"/>
        <!-- Not needed because of tunnel-->
<!--        <xsl:param name="a-head"/>
        <xsl:param name="b-head"/>-->
        ...
        <xsl:copy>
            ...
            <xsl:apply-templates select="node()" mode="get_first_chunk">
                <xsl:with-param name="chunk_id" select="$chunk_id"/>
                <xsl:with-param name="counter" select="$counter"/>
                <xsl:with-param name="chunk_map" select="$chunk_map"/>
                <xsl:with-param name="topic_id" select="$topic_id"/>
    ...
    <xsl:template match="node()" mode="get_first_chunk">
        <xsl:copy>
            <xsl:apply-templates select="@*|node()" mode="get_first_chunk"/>
        </xsl:copy>
    </xsl:template>
    ...
    <xsl:template match="@*" mode="chunk_topic get_first_chunk">
        <xsl:copy/>
    </xsl:template>
    ...
    <xsl:template match="*[contains(@class,' topic/body ')]" mode="chunk_topic" priority="100">
        ...
        <xsl:copy>
            ...
            <xsl:if test="$counter = 1">
                <h3><xsl:value-of select="$b-head"/></h3>
            </xsl:if>
<!--            <xsl:if test="$counter = 2 and $chunk_map/initial/@empty = 'yes'">
                <h3><xsl:value-of select="$b-head"/></h3>
            </xsl:if>XYZZY-->
...
<!--                            <xsl:comment>before_next_break_or_end_1 contains <xsl:value-of select="count($before_next_break_or_end_1)"/> elements.</xsl:comment>-->
                    ...
                    <xsl:choose>
                        ...
                        <xsl:when test="section">
                            <xsl:apply-templates select="*[not(self::section)]" mode="chunk_topic_identity">
                                <xsl:with-param name="b-head" select="$b-head"/>
    ...
    <!-- Body: where (most) everything happens. -->
    <!-- Just need the first chunk. -->
    <xsl:template match="*[contains(@class,' topic/body ')]" mode="get_first_chunk" priority="100">
        <xsl:param name="chunk_id"/>
        <xsl:param name="counter"/>
        <xsl:param name="chunk_map" as="element()*"/>
        <xsl:param name="topic_id"/>
        ...
        <xsl:copy>
            ...
            <xsl:choose>
                <!-- No sections or breaks, just use identity transform. -->
                <xsl:when test="not(*[contains(@class,' topic/section ')]) and
                    not(descendant-or-self::data[@name='pace_break' and upper-case(@value)='Y'])">
                    
                    <xsl:apply-templates mode="chunk_topic_identity"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:variable name="break_pos" 
                        select="count(*[descendant-or-self::data[@name='pace_break' and upper-case(@value)='Y' and generate-id() = $chunk_id]]/preceding-sibling::*)+1"/>
                    
                    <xsl:variable name="section_pos"
                        select="count(*[contains(@class,' topic/section ')][1]/preceding-sibling::*)+1"/>
                    <xsl:choose>
                        <xsl:when test="$break_pos gt 1 and $break_pos lt $section_pos">
                            <!-- process up to break. -->
                            <xsl:apply-templates select="*[position() lt $break_pos]" mode="chunk_topic_identity"/>
                        </xsl:when>
                        <xsl:otherwise>
                            <!-- process up to section. -->
                            
                            <xsl:apply-templates select="*[position() lt $section_pos]" mode="chunk_topic_identity"/>
                        </xsl:otherwise>
                    </xsl:choose>
    ...
    <xsl:template match="*[contains(@class,' topic/title ') and (parent::section or parent::*[contains(@class,' topic/topic ')])]" mode="chunk_topic" priority="100">
        ...
        <xsl:param name="a-head" tunnel="yes"/>
        <xsl:param name="b-head" tunnel="yes"/>
<!--        <xsl:message>in title (parent is <xsl:value-of select="name(parent::*)"/>): a-head is: <xsl:value-of select="$a-head"/>.</xsl:message>
        <xsl:message>in title (parent is <xsl:value-of select="name(parent::*)"/>): b-head is: <xsl:value-of select="$b-head"/>.</xsl:message>
-->        
        <xsl:copy>
            <xsl:apply-templates select="@*" mode="chunk_topic"/>
            
            <xsl:choose>
                <xsl:when test="parent::section and not(empty($b-head))">
                    <xsl:value-of select="$b-head"/>
                </xsl:when>
                <xsl:when test="not(empty($a-head))">
                    <xsl:value-of select="$a-head"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:apply-templates select="node()" mode="chunk_topic">
                        <xsl:with-param name="chunk_id" select="$chunk_id"/>
            ...
            <xsl:if test="($counter gt 1 or not(empty($a-head))) and not(contains($a-head,'(cont.)'))">
                <xsl:text> (cont.)</xsl:text>            
            </xsl:if>
        </xsl:copy>
    </xsl:template>
    
    <!-- Handle title for first chunk.-->
    <xsl:template match="*[contains(@class,' topic/title ') and
            (parent::section or parent::*[contains(@class,' topic/topic ')])]" 
            mode="get_first_chunk" priority="100">
        <xsl:param name="chunk_id"/>
        <xsl:param name="counter"/>
        <xsl:param name="chunk_map" as="element()*"/>
        <xsl:param name="topic_id"/>
        ...
        <xsl:comment>in title for get_first_chunk</xsl:comment>
        <xsl:copy>
            <xsl:apply-templates select="@*" mode="chunk_topic"/>
            <xsl:value-of select="$a-head"/>
...
<!--            <xsl:comment>section (chunk_topic) b-head-extra is <xsl:value-of select="$b-head"/></xsl:comment>
            <xsl:message>section (chunk_topic) b-head-extra is <xsl:value-of select="$b-head"/></xsl:message>-->
            <xsl:choose>
...
<!--                            <xsl:comment>break_elements contains <xsl:value-of select="count($break_elements)"/> elements.</xsl:comment>-->
                        ...
                        <xsl:otherwise>
                            <xsl:choose>
                                <xsl:when test="*[contains(@class,' topic/title ')]"> 
                                    ...
                                    <xsl:apply-templates select="*[contains(@class,' topic/title ')]"
                                        mode="chunk_topic_identity">
                                        <xsl:with-param name="b-head" select="$b-head"/>
                                        <xsl:with-param name="show-cont">
                                            <xsl:choose>
                                                <xsl:when test="$counter = 2 and $chunk_map/initial/@empty = 'yes'">
                                                    <xsl:value-of select="'false'"/>
                                                </xsl:when>
                                                <xsl:otherwise>
                                                    <xsl:value-of select="'true'"/>
                            ...
                            <xsl:apply-templates select="node()[not(contains(@class,' topic/title '))]" mode="chunk_topic_identity">
                                <xsl:with-param name="b-head" select="$b-head"/>
                            </xsl:apply-templates>
                        </xsl:otherwise>
                    </xsl:choose>
                ...
                <xsl:when test="name($chunk_map_element) = 'break'">
                    <xsl:comment>In section, looking for break. </xsl:comment>
...
<!--                            <xsl:comment>break_elements contains <xsl:value-of select="count($break_elements)"/> elements.</xsl:comment>-->
    ...
    <!--  -->
    <!-- [SP] 2020-12-08 sfb: One exception to identity: the title.-->
    <xsl:template match="*[contains(@class,' topic/title ')]" mode="chunk_topic_identity">
        <xsl:param name="b-head"/>
        <xsl:param name="show-cont" select="'undefined'"/>
<!--        <xsl:comment>In title, chunk_topic_identity</xsl:comment>
        <xsl:comment>$show-cont is <xsl:value-of select="$show-cont"/></xsl:comment>-->
        <xsl:choose>
            <xsl:when test="not(empty($b-head))">
                <xsl:copy>
                    <xsl:apply-templates select="@*" mode="chunk_topic_identity"/>
                    <xsl:value-of select="$b-head"/>
                    <!-- XYZZY Here's the big problem: if counter is 2, but the first "section" is empty, 
                          The (cont.) shouldn't appear. But we have to know that when this gets called.
                        -->
                    <xsl:if test="$show-cont = ('true','undefined')">
                        <xsl:text> (cont.)</xsl:text>
                    </xsl:if>
                </xsl:copy>
                <!-- Now apply the title. -->
                <p><strong><xsl:apply-templates select="@*|node()" mode="chunk_topic_identity"/></strong></p>
            </xsl:when>
            <xsl:otherwise>
                <xsl:copy>
                    <xsl:apply-templates select="@*|node()" mode="chunk_topic_identity"/>
