...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="supermap">
        <map>
            ...
            <xsl:apply-templates select="topichead[@type='final-exam' and examMap/@assessment_type='test_exam_primary']/examMap"/>
    ...
    <!-- Top level topichead. -->
    <xsl:template match="topichead[not(@type='final-exam')]">
        ...
        <xsl:copy>
            ...
            <xsl:apply-templates select="*[not(self::examMap[@assessment_type='test_exam_primary']) and not(self::navtitle)]">
    ...
    <xsl:template match="topichead[@type='final-exam']">
        
        <!-- Create the related files. -->
        <xsl:variable name="topic" select="document(@task_href)/*" as="element()*"/>
        
        <!-- Create the task file from the task_href. -->
...
<!--            <xsl:with-param name="task_category" select="'test_exam_primary'"/>-->
            <xsl:with-param name="task_category" select="'default'"/>
        </xsl:call-template>
...
<!--        <xsl:variable name="task_number" select="count(preceding-sibling::topicref[@task_category!='page_turner']) + 1"/>-->
        ...
        <xsl:variable name="lesson_number">
            <xsl:if test="parent::topichead[starts-with(@navtitle,'Lesson')]">
                <xsl:value-of select="replace(parent::topichead/@navtitle,'^Lesson\s([^:]*): .*$','$1')"/>
            </xsl:if>
        </xsl:variable>
...
<!--                    <xsl:with-param name="topic_number" select="$topic_number"/>-->
            ...
            <!-- [SP] 2015-04-16 sfb: Handle topicrefs for exams. -->
            <xsl:when test="@task_category='default' and @type='exam'">
<!--                <xsl:call-template name="new_topic">
                    <xsl:with-param name="topic" select="$topic"/>
                    <!-\-                    <xsl:with-param name="name_base" select="ancestor::topichead[@name_base][1]/@name_base"/>-\->
                    <xsl:with-param name="unit" select="ancestor::topichead[@unit][1]/@unit"/>
                ...
                <!-- Handle the examMap. -->
                <xsl:apply-templates select="examMap"/>
...
<!--                    <xsl:for-each select="following-sibling::examMap[@assessment_type!='test_exam_primary']">-->
...
<!--                    <xsl:with-param name="name_base" select="ancestor::topichead[@name_base][1]/@name_base"/>-->
                    <xsl:with-param name="unit" select="ancestor::topichead[@unit][1]/@unit"/>
<!--                    <xsl:with-param name="topic_number" select="$topic_number"/>-->
                    <xsl:with-param name="navtitle" select="$navtitle"/>                </xsl:call-template>
            </xsl:when>
...
<!--                    <xsl:with-param name="name_base" select="ancestor::topichead[@name_base][1]/@name_base"/>-->
        ...
        <!-- Handle nested topicrefs for concepts. -->
        <xsl:apply-templates select="topicref | task-chunk | examMap[@assessment_type='test_topic_practice']"/>
        
    </xsl:template>
    
    
    <!-- Handle task-chunk. We know this is a pointer to a task, so we can avoid lots of decisions. -->
    <xsl:template match="task-chunk">
        
        <xsl:message>Handling task-chunk. </xsl:message>
        <!-- Get the category number. -->
        <xsl:variable name="this_category" select="@task_category"/>
...
<!--            <xsl:with-param name="topic_number" select="$topic_number"/>-->
            <xsl:with-param name="task_category" select="@task_category"/>
            <xsl:with-param name="show_duration" select="false()"/>
        </xsl:call-template>
        
        <!-- We don't chunk tasks, just the topics created from the tasks. -->
        <!--        <!-\- Create the new task. -\->
        <xsl:call-template name="new_task">
            <xsl:with-param name="topic" select="$topic"/>
            <xsl:with-param name="unit" select="ancestor::topichead[@unit][1]/@unit"/>
...
<!--        <xsl:param name="all_exam_maps" as="element()*"/>-->
        ...
        <xsl:if test="not(preceding-sibling::topicref[@type='final-exam']) 
            or @assessment_type='test_exam_primary'">
            <!--            or (@assessment_type='test_exam_primary' and ancestor::supermap[@category='default'])">-->
...
<!--                <xsl:with-param name="exam_number" select="$exam_map_number"/>-->
    ...
    <xsl:template name="get_file_id_base">
        <xsl:param name="orig_href" select="@orig_href"/>
...
<!--        <xsl:param name="topic_number"/>-->
        ...
        <xsl:param name="show_duration" select="true()"/>
        ...
        <xsl:result-document href="{$out_file}"     
            ...
            <xsl:apply-templates select="$topic" mode="new_topic">
                <xsl:with-param name="unit" select="$unit"/>
                <xsl:with-param name="task_category" select="$task_category"/>
<!-- [SP] 2015-04-20 sfb: Trying to resolve ID issues. (Gotta be case insensitive, too.) -->
                <xsl:with-param name="topic_id" select="replace($file_id_base,'TASK','TASK-TOPIC','i')"/>
                <!--                <xsl:with-param name="topic_id" select="$file_id_base"/>-->
                <xsl:with-param name="show_duration" select="$show_duration"/>
               <xsl:with-param name="navtitle" select="$navtitle"/>
            </xsl:apply-templates>
        </xsl:result-document>
...
<!--        <xsl:param name="category_number"/>-->
        ...
        <xsl:variable name="file_id_base">
            <xsl:call-template name="get_file_id_base">
                <xsl:with-param name="orig_href">
                    <xsl:choose>
                        <xsl:when test="@type = 'final-exam'">
                            <xsl:value-of select="@task_href"/>
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:value-of select="@orig_href"/>
                        </xsl:otherwise>
                    </xsl:choose>
                </xsl:with-param>
            </xsl:call-template>
...
<!--        <xsl:param name="name_base"/>-->
        ...
        <xsl:result-document href="{$out_file}" method="xml" indent="yes" doctype-public="''" doctype-system="''">
            ...
            <xsl:apply-templates select="$topic" mode="new_assessment">
                <xsl:with-param name="unit" select="$unit"/>
                <xsl:with-param name="topic_id" select="replace($file_id_base,'TASK','QOVERVIEW','i')"/>
            </xsl:apply-templates>
        </xsl:result-document>
...
<!--        <xsl:param name="name_base"/>-->
        ...
        <xsl:result-document href="{$out_file}" method="xml" indent="yes" doctype-public="''" doctype-system="''">
            ...
            <xsl:apply-templates select="$topic" mode="new_assessment_from_q">
                <xsl:with-param name="unit" select="$unit"/>
                <xsl:with-param name="topic_id" select="replace($file_id_base,'TASK','QUESTION','i')"/>
                <xsl:with-param name="question_number" select="$question_number"/>
            </xsl:apply-templates>
        </xsl:result-document>
    ...
    <!-- I suspect that this code is no longer used. -->
...
<!--        <xsl:variable name="file_name" select="replace($file_id_base,'^(.*)_(.*)_(.*)_(\d*)(.*)$','questions/$1_overview_$3_$4$5.dita')"/>-->
        ...
        <xsl:variable name="exam_type">
            <xsl:choose>
                <xsl:when test="$assessment_type = 'test_exam_primary'">
...
<!--        <xsl:param name="topic_number"/>-->
        ...
        <xsl:message>file_id_base is: <xsl:value-of select="$file_id_base"/>.</xsl:message>
        <xsl:variable name="file_name" select="concat('overview/',$file_id_base,'.dita')"/>
...
<!--        <xsl:param name="topic_number"/>-->
        ...
        <xsl:choose>
            <xsl:when test="@type='final-exam'">
                <xsl:apply-templates select="topicsubject" mode="reltable">
                    <xsl:with-param name="type" select="'final-exam'"/>                    
                </xsl:apply-templates>
            </xsl:when>
            <xsl:otherwise>
    ...
    <!-- Essentially handles @task_category = 'default', in addition to any other "left-overs".  -->
    <xsl:template match="topicref[@task_category != 'assignment' 
                              and @task_category != 'exam' 
                              and @task_category != 'kpe-concept' 
                              and @task_category != 'kpe-question']" mode="reltable">
        
<!--        <xsl:comment>default topicref case (@orig_href = <xsl:value-of select="@orig_href"/>).</xsl:comment>-->
...
<!--            <xsl:with-param name="task_number" select="$task_number"/>-->
            <xsl:with-param name="type">
                <xsl:choose>
                    ...
                    <!-- [SP] 2015-04-07: Necessary for nested concepts. -->
        ...
        <!-- Nested topicrefs (concepts). -->
        <xsl:apply-templates select="topicref | task-chunk  | examMap[@assessment_type='test_topic_practice']" mode="reltable"/>
        
    </xsl:template>
    
    <!-- Handle examMaps for test_topic_practice.   -->
    <xsl:template match="examMap[@assessment_type='test_topic_practice']" mode="reltable">
        
        <!-- Nested topicrefs (concepts). -->
        <xsl:apply-templates select="topicref | task-chunk" mode="reltable"/>
        
    </xsl:template>
    
    <!-- Essentially handles @task_category = 'default', in addition to any other "left-overs".  -->
    <xsl:template match="topicref[@task_category = 'kpe-concept']" mode="reltable">
        <xsl:apply-templates select="topicsubject" mode="reltable">
            <xsl:with-param name="type">
                <xsl:choose>
                    <!-- [SP] 2015-04-07: Necessary for nested concepts. -->
    ...
    <xsl:template match="task-chunk" mode="reltable">
        
        <xsl:apply-templates select="parent::*/topicsubject" mode="reltable">
            <xsl:with-param name="type" select="'task-chunk'"/>
            <!-- Because of the task-chunk's unique location, we have to preset the parent_href and file_id_base. -->
            <xsl:with-param name="parent_href" select="@orig_href"/>
            <xsl:with-param name="file_id_base" select="replace(@orig_href,'.*/([^/]*)\.dita','$1')"/>
            <xsl:with-param name="task_file_id_base" select="replace(parent::*/@orig_href,'.*/([^/]*)\.dita','$1')"/>
        </xsl:apply-templates>
        
        <!-- Nested topicrefs? Don't think there should be any. -->
        <xsl:apply-templates select="topicref | task-chunk | examMap[@assessment_type='test_topic_practice']" mode="reltable"/>
...
-->
        <xsl:apply-templates select="topicsubject" mode="reltable">
            ...
            <xsl:with-param name="type">
                <xsl:choose>
                    <xsl:when test="parent::examMap[@assessment_type='test_topic_practice']">
                        <xsl:text>test-topic-practice</xsl:text>
                    </xsl:when>
                    <xsl:otherwise>
...
<!--        <xsl:param name="task_number"/>-->
        ...
        <!-- If we're calling this for a task-chunk, the parent_href and file_id_base are passed in. -->
        <xsl:param name="parent_href" select="parent::*/@orig_href"/>
        <xsl:param name="file_id_base">
            <xsl:call-template name="get_file_id_base">
                <xsl:with-param name="orig_href" select="$parent_href"/>
            </xsl:call-template>
        </xsl:param>
        <xsl:param name="task_file_id_base"/>
...
<!--        <xsl:comment>original href of parent is <xsl:value-of select="$parent_href"/>.</xsl:comment>-->
        
<!--        <xsl:comment>file_id_base is: <xsl:value-of select="$file_id_base"/>.</xsl:comment>-->
        ...
        <relrow>
           ...
           <relcell>
               <topicref>
                   <xsl:attribute name="href">
                       <xsl:choose>
                           <xsl:when test="$type = 'exam' or $type = 'final-exam'">
                               <xsl:variable name="exam_file_id_base">
                                   <xsl:call-template name="get_file_id_base">
                                       <xsl:with-param name="orig_href" select="following-sibling::examMap/@orig_href"/>
                                   </xsl:call-template>
                               </xsl:variable>
                               
                               <xsl:value-of select="concat('questions/',$exam_file_id_base,'.dita')"/>
                           </xsl:when>
                           <xsl:when test="$type = 'test-topic-practice'">
                               <xsl:variable name="exam_file_id_base">
                                   <xsl:call-template name="get_file_id_base">
                                       <xsl:with-param name="orig_href" select="ancestor::examMap/@orig_href"/>
                                   </xsl:call-template>
                               </xsl:variable>
           ...
           <relcell>
               <topicref>
                   <xsl:choose>
                       <xsl:when test="$type = 'final-exam'">
                           <xsl:variable name="task_file_id_base">
                               <xsl:call-template name="get_file_id_base">
                                   <xsl:with-param name="orig_href" select="parent::*/@task_href"/>
                               </xsl:call-template>
                           </xsl:variable>
                           <xsl:attribute name="href">                           
                               <xsl:value-of select="concat('tasks/',$task_file_id_base,'.dita')"/>
                           </xsl:attribute>
                       ...
                       <xsl:when test="$type = 'test-topic-practice'">
                           <xsl:variable name="task_file_id_base">
                               <xsl:call-template name="get_file_id_base">
                                   <xsl:with-param name="orig_href" select="ancestor::topicref[@type='topic']/@orig_href"/>
                               </xsl:call-template>
                           </xsl:variable>
                           <xsl:attribute name="href">
                               <xsl:value-of select="replace($task_file_id_base,'^(.*)_(.*)_(.*)_(\d*)(.*)$','tasks/$1_task_$3_$4$5.dita')"/>                               
<!--                               <xsl:value-of select="concat('tasks/',$task_file_id_base,'.dita')"/>-->
                           </xsl:attribute>
                           
                       </xsl:when>
                       <xsl:when test="$type='task-chunk'">
<!--                           <xsl:attribute name="href">
                               <xsl:value-of select="concat('topics/',$task_file_id_base,'.dita')"/>
                           </xsl:attribute>-->
                           <xsl:variable name="parent_id_base">
                               <xsl:call-template name="get_file_id_base">
                                   <xsl:with-param name="orig_href" select="parent::*/@orig_href"/>
                               </xsl:call-template>
                           </xsl:variable>
                           <xsl:attribute name="href">
                               <xsl:value-of select="replace($parent_id_base,'^(.*)_(.*)_(.*)_(\d*)(.*)$','tasks/$1_task_$3_$4$5.dita')"/>
                           </xsl:attribute>
                       </xsl:when>
                       <xsl:when test="$type='kpe-concept'">
                           <!-- [SP] 2015-04-07: LMS V3 Calendar work: for nested concept, pick up the containing task. -->
                           <xsl:variable name="parent_id_base">
                               <xsl:call-template name="get_file_id_base">
                                   <xsl:with-param name="orig_href" select="parent::*/parent::*/@orig_href"/>
                               </xsl:call-template>
                           </xsl:variable>
                           <xsl:attribute name="href">
                                <xsl:value-of select="replace($parent_id_base,'^(.*)_(.*)_(.*)_(\d*)(.*)$','tasks/$1_task_$3_$4$5.dita')"/>
                       ...
                       <xsl:when test="$type = 'kpe-question'">
                           ...
                           <xsl:message>IS THIS USED ANYMORE?</xsl:message>
                           <xsl:comment>IS THIS USED ANYMORE?</xsl:comment>
                       </xsl:when>
                       <xsl:otherwise>
                           <xsl:attribute name="href">
                               <xsl:value-of select="replace($file_id_base,'^(.*)_(.*)_(.*)_(\d*)(.*)$','tasks/$1_task_$3_$4$5.dita')"/>                               
           ...
           <relcell>
               <topicref>
                   <xsl:attribute name="href">
                       <xsl:variable name="los_id_base" select="replace(@orig_href,'.*/([^/]*)\.dita','$1')"/>
                       
                       <xsl:value-of select="concat('los/',$los_id_base,'.dita')"/>
               </xsl:attribute>
               </topicref>
           </relcell>
        </relrow>
