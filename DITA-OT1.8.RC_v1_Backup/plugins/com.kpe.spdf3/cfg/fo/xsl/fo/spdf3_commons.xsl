        ...
        <fo:page-sequence master-reference="body-sequence"
            ...
            <fo:flow flow-name="xsl-region-body">
                <fo:block xsl:use-attribute-sets="topic">
                    ...
                    <xsl:apply-templates select="*[contains(@class,' topic/prolog ')]"/>
                    <!-- [SP] No need for chapter static content in generic spdf3 format. -->
...
-->
                    <fo:block xsl:use-attribute-sets="topic.title">
                        ...
                        <fo:block>
                            ...
                            <xsl:for-each select="child::*[contains(@class,' topic/title ')]">
                                <xsl:apply-templates select="." mode="getTitle"/><!-- xxxxx -->
                            </xsl:for-each>
                        </fo:block>
                    </fo:block>
        ...
        <fo:page-sequence master-reference="body-sequence"
            ...
            <fo:flow flow-name="xsl-region-body">
                <fo:block xsl:use-attribute-sets="topic">
                    ...
                    <xsl:variable name="part_image_href"
                        select="$part_image/@href"/>
                    ...
                    <xsl:apply-templates select="*[contains(@class,' topic/prolog ')]"/>
                    <!-- [SP] No need for part static content in generic spdf3 format. -->
                    <!--				    <xsl:call-template name="insertChapterFirstpageStaticContent">
...
<!--        <xsl:comment>Level is: <xsl:value-of select="$level"/>.</xsl:comment>-->
        ...
        <fo:block>
            ...
            <xsl:call-template name="processAttrSetReflection">
                ...
                <xsl:with-param name="path" select="'../../cfg/fo/attrs/spdf3_commons-attr.xsl'"/>
            </xsl:call-template>
...
<!--            <xsl:when test="$topicType = 'topicColophon'">
    ...
    <xsl:template match="*[contains(@class, ' bookmap/chapter ')]" mode="topicTitleNumber" priority="-1">
        ...
        <xsl:variable name="quiz_count" 
            select="count(preceding::*[contains(@class, ' bookmap/chapter ')]
    ...
    <xsl:template match="*[contains(@class, ' topic/object ')]">
        ...
        <fo:block xsl:use-attribute-sets="p.object" id="{@id}">
            ...
            <xsl:value-of select="tokenize(@data, '/')[last()]"/>
            <xsl:text>]</xsl:text>
            <xsl:apply-templates/>
        </fo:block>
    </xsl:template>
        ...
        <xsl:choose>
            ...
            <xsl:when test="parent::*[contains(@class,' topic/lq ')]">
                <fo:block xsl:use-attribute-sets="lq" id="{@id}">
                    ...
                    <xsl:choose>
                        <xsl:when
                            test="following-sibling::*[contains(@class, ' topic/lq ')] 
    ...
    <!-- [SP] Eliminate OT warnings by providing a default image alignment value if none is provided. -->
        ...
        <xsl:variable name="imageAlignTested">
            <xsl:choose>
                ...
                <!-- If not specified, use the $image-align value from spdf3_BasicSettings.xsl. -->
