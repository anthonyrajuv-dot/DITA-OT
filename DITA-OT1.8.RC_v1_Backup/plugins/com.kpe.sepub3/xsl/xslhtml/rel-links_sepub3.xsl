...
<xsl:stylesheet version="1.0" 
  ...
  exclude-result-prefixes="related-links ditamsg #default">
...
<!-- [SP] turn off insertion of parent links SSO -->
<xsl:param name="NOPARENTLINK" select="'yes'"/><!-- "no" and "yes" are valid values; non-'no' is ignored -->
<xsl:param name="include.rellinks" select="'#default parent sibling friend next previous cousin ancestor sample external other'"/>
...
<!-- [SP] no child link processing, please SSO -->
    <!-- [SP] 01-Feb-2013: Yes, we need it for scriptorium. -->
...
<!-- [SP] 01-Feb-2013: Commented out, because XHTML template does what we need. -->
...
<xsl:template match="*" mode="add-link-target-attribute">
  <xsl:if test="@scope='external' or @type='external' or ((@format='PDF' or @format='pdf') and not(@scope='local'))">
    <!-- [SP] epubs don't need blank targets -->
