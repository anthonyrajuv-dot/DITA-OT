...
-->
  ...
  <xsl:template match="*[contains(@class, ' topic/topic ')]" mode="child.topic" name="child.topic">
    ...
    <!-- [SP] 2015-02-25: Moved the div for non-sepub3 only. -->
      <xsl:choose>
...
<!--        <xsl:call-template name="commonattributes">
    ...
    <xsl:choose>
      <xsl:when test="contains(@class,' kpe-overview/kpe-overview ')">
        <xsl:comment>Got kpe-overview.</xsl:comment>
        <div id="{@id}"/>
        <xsl:apply-templates/>    
      </xsl:when>
      <xsl:when test="contains(@class,' kpe-summary/kpe-summary ')">
        <xsl:comment>Got kpe-summary.</xsl:comment>
        <div id="{@id}"/>
        <xsl:apply-templates/>    
      </xsl:when>
      <xsl:when test="contains(@class,' kpe-concept/kpe-concept ') and not(parent::dita)">
        <xsl:comment>Got nested kpe-concept.</xsl:comment>
        <div id="{@id}"/>
        <xsl:apply-templates/>    
      </xsl:when>
      <xsl:when test="contains(@class,' kpe-assessmentOverview/kpe-assessmentOverview ')">
        ...
        <!-- Add the answer key. -->
        <xsl:apply-templates select="." mode="answer_key_output"/> 
      </xsl:when>
...
<!--  PSB 7/31/15 Added to include Open Questions-->
  ...
  <xsl:template match="*[contains(@class,' topic/title ')]" mode="answer_key_output">
    <p class="unit_title"><xsl:apply-templates/> Answer Key</p>
  </xsl:template>
  ...
  <!-- [SP] 2015-02-26: Utility template to test if an element contains mixed content elements.-->
<!--    %p; | %lq; | %note; | %dl; | %ul; | %ol;|  
    %sl; | %pre; | %lines; | %fig; | %image; | 
    %object; |  %table; | %simpletable;-->
  <xsl:template name="has_basic_block">
    <xsl:choose>
  ...
  <xsl:template match="*[contains(@class,' topic/title ')]" mode="topic_title_sepub3">
    ...
    <xsl:choose>
      <xsl:when test="parent::*[contains(@class,' kpe-question/kpe-question ')]">
        <!-- Do nothing. -->
      </xsl:when>
      <xsl:when test="ancestor::*[contains(@class,' kpe-glossEntry/kpe-glossEntry ')]">
        <p>
          <b>
            <xsl:call-template name="check_navtitle"/>
          </b>
        </p>
      </xsl:when>
      <xsl:when test="parent::*[contains(@class,' topic/topic ') and @outputclass='topic_intro']">
        <p class="unit_title">
          <xsl:call-template name="check_navtitle"/>
        </p>
      </xsl:when>
  ...
  <xsl:template name="check_navtitle">
    <xsl:variable name="my_id" select="parent::*/@id"/>
    <xsl:variable name="my_topicref" 
      select="$map/descendant::*[contains(@class,' map/topicref ') and contains(@href,$my_id)]"/>
    <xsl:choose>
      <xsl:when test="$my_topicref/*[contains(@class,' map/topicmeta ')]/*[contains(@class,' topic/navtitle ')]">
        <xsl:apply-templates select="$my_topicref/*[contains(@class,' map/topicmeta ')]/*[contains(@class,' topic/navtitle ')]"/>
      </xsl:when>
      <xsl:otherwise>
        <xsl:apply-templates/>
      </xsl:otherwise>
...
-->
  ...
  <!-- paragraphs -->
  <xsl:template match="sample[contains(@othertype,'profnote')]/p[1]">
  <!--  <xsl:choose>
      <xsl:when test="p[1]">-->
        <img class="prof_hat" src="../Images/profhat.png"/>
        <p class="example_body_note">
          <xsl:apply-templates/>
        </p>
     <!-- </xsl:when>-->
  </xsl:template>
     ...
     <xsl:choose>
      <xsl:when test="parent::*[contains(@class,' topic/fig ')]">
        <p class="figure_body">
          ...
          <!--<xsl:when test="parent::*[contains(@class,' kpe-common-d/sample ')][contains(@othertype,'profnote')]">
            <xsl:choose>
              <xsl:when test="p[1]">
                <img class="prof_hat" src="../Images/profhat.png"/>
                <p class="example_body_note">
          ...
          </xsl:when>--><!--[not(contains(@othertype,'profnote'))]-->
...
<!--  PSB COMMENTED THESE OUT TO REMOVE EXTRA TABLE JUNK    -->
      <!--<xsl:choose>
...
<!--this gets rid of align in the thead-->
<xsl:template match="thead">
  ...
  <!-- ================ GLOSSARY ================ -->
  <xsl:template match="*[contains(@class,' glossentry/glossdef ')]" mode="glossentry_glossdef.sepub3">
    <p class="glossdef">
      <xsl:apply-templates/>
    </p>
  </xsl:template>
 ...
 +-->
  ...
  <xsl:template match="*" mode="chapterBody_sepub3">
    ...
    <body>
      ...
      <xsl:call-template name="setidaname"/>
      <!-- [SP] 2015-02-25: Add anchor by hand for  kpe-concept/kpe-concept -->
      <!--      <xsl:comment>Adding anchor; class is <xsl:value-of select="@class"/>.</xsl:comment>-->
      <xsl:choose>
        <xsl:when test="*[contains(@class,' kpe-concept/kpe-concept ')]">
          <xsl:attribute name="id">
            <xsl:value-of select="*[contains(@class,' kpe-concept/kpe-concept ')][1]/@id"/>
          </xsl:attribute>
          <!--          <a name="{*[contains(@class,' kpe-concept/kpe-concept ')][1]/@id}"/>-->
        </xsl:when>
      </xsl:choose>
      <xsl:value-of select="$newline"/>
      <xsl:call-template name="start-flags-and-rev">
  ...
  <!-- [SP] 2015-02-26: ensure question is tagged.-->
  <xsl:template match="*[contains(@class,' learning2-d/lcQuestion2 ')]" priority="100">
    <xsl:variable name="has_basic_block">
      <xsl:call-template name="has_basic_block"/>
    </xsl:variable>
    
    <!-- TODO: handle text() followed by basic-block character. -->
    <xsl:choose>
      <xsl:when test="$has_basic_block = 'true'">
        <xsl:apply-templates/>        
      </xsl:when>
      <xsl:otherwise>
        <p>
        ...
        <li>
          <xsl:choose>
            <xsl:when test="*[' learning2-d/lcAnswerContent2 ']">
              <xsl:attribute name="class">
                <xsl:text>q_distractor correct</xsl:text>
              </xsl:attribute>
            </xsl:when>
          </xsl:choose>
          <xsl:apply-templates/>
...
<!--PSB 7/31/15 Added lcOpenAnswer2-->
  ...
  <!-- Override processing for  kpe-assessmentOverview/kpe-assessmentOverviewBody -->
  <xsl:template match="*[contains(@class,' kpe-assessmentOverview/kpe-assessmentOverviewBody ')]">
    <!-- Do nothing. -->
  </xsl:template>
